...
- 自研文件使用 `vp_` 加小写下划线，例如 `vp_cad_document.cpp`。
- 类、结构体、枚举类型和类型别名使用`Vp` 前缀，例如 `VpCadDocument`。
- 接口类型使用 `VpI` 前缀，例如 `VpIFileCodec`。
...
- 拆分实现文件时保持类的公共接口稳定，并在所属模块的 `vp_*.vcxproj` 中显式登记
  新文件，同时维护 `.vcxproj.filters` 分组。含 `Q_OBJECT` 的文件按需使用 `QtMoc`，
  `.qrc` 使用 `QtRcc`；第三方源码保持上游原貌，不为满足本限制直接修改。
...
- 公共 API 不传播异常，失败结果通过 `VpResult<T>` 返回。
- 注释解释原因和约束，不复述代码。
