...
class SCadDocument final : public QObject
{
    Q_OBJECT

  public:
    explicit SCadDocument(QObject* parent = nullptr);

    const std::vector<SEntityRecord>& entities() const noexcept;
    QString filePath() const;
    QString displayName() const;
    bool isModified() const noexcept;
    bool canUndo() const noexcept;
    bool canRedo() const noexcept;
    const SDrawingSettings& drawingSettings() const noexcept;
    QStringList layerNames() const;
    const std::vector<SLayerRecord>& layers() const noexcept;
    const SLayerRecord* layer(const QString& layer_name) const noexcept;
    ...
    QString assignEntitiesToColorLayer(const std::vector<SEntityId>& entity_ids,
                                       const QColor& color);
    ...
    const std::vector<STextStyleRecord>& textStyles() const noexcept;
    const STextStyleRecord* textStyle(const QString& style_name) const noexcept;
    QString currentTextStyleName() const;
    bool addOrUpdateTextStyle(const STextStyleRecord& text_style);
    bool removeTextStyle(const QString& style_name);
    bool setCurrentTextStyle(const QString& style_name);
    const std::vector<SDimensionStyleRecord>& dimensionStyles() const noexcept;
    const SDimensionStyleRecord* dimensionStyle(const QString& style_name) const noexcept;
    QString currentDimensionStyleName() const;
    bool addOrUpdateDimensionStyle(const SDimensionStyleRecord& dimension_style);
    bool removeDimensionStyle(const QString& style_name);
    bool setCurrentDimensionStyle(const QString& style_name);

    std::unique_ptr<SDocumentTransaction> beginTransaction(const QString& label);
    SResult<void> save(const QString& file_path);
    SResult<void> saveRecoveryCopy(const QString& file_path);
    SResult<void> load(const QString& file_path);
    SResult<void> recover(const QString& recovery_file_path, const QString& original_file_path);
    SDocumentAuditReport audit(bool repair);
    void clear();
    void undo();
    void redo();
  ...
  private:
    ...
    struct SHistoryEntry
    {
        ...
        std::vector<SLayerRecord> layers_before;
        std::vector<SLayerRecord> layers_after;
        QString current_layer_before;
        QString current_layer_after;
        bool has_drawing_settings_change = false;
        SDrawingSettings drawing_settings_before;
        SDrawingSettings drawing_settings_after;
        bool has_text_style_change = false;
        std::vector<STextStyleRecord> text_styles_before;
        std::vector<STextStyleRecord> text_styles_after;
        QString current_text_style_before;
        QString current_text_style_after;
        bool has_dimension_style_change = false;
        std::vector<SDimensionStyleRecord> dimension_styles_before;
        std::vector<SDimensionStyleRecord> dimension_styles_after;
        QString current_dimension_style_before;
        QString current_dimension_style_after;
        bool has_entity_order_change = false;
        std::vector<SEntityId> entity_order_before;
        std::vector<SEntityId> entity_order_after;
    };

    SEntityId reserveEntityId() noexcept;
    SLayerId reserveLayerId() noexcept;
    QString resolveDrawingLayerName();
    void ensureDefpointsLayer();
    SLayerRecord* mutableLayer(const QString& layer_name) noexcept;
    void commitEntities(QString label, std::vector<SEntityRecord> added_entities,
                        std::vector<SEntityRecord> removed_entities,
                        std::optional<SDrawingSettings> drawing_settings = std::nullopt,
                        std::optional<std::vector<SEntityId>> entity_order = std::nullopt);
    void commitLayerChange(QString label, std::vector<SLayerRecord> layers_before,
                           QString current_layer_before, std::vector<SEntityRecord> added_entities,
                           std::vector<SEntityRecord> removed_entities);
    void setModified(bool is_modified);
    void emitDocumentState();
    void commitTextStyleChange(QString label, std::vector<STextStyleRecord> styles_before,
                               QString current_style_before);
    void commitDimensionStyleChange(QString label, std::vector<SDimensionStyleRecord> styles_before,
                                    QString current_style_before);
    SResult<void> saveInternal(const QString& file_path, bool update_document_state);

    std::vector<SEntityRecord> m_entities;
    std::vector<SHistoryEntry> m_undo_stack;
    std::vector<SHistoryEntry> m_redo_stack;
    SEntityId m_next_entity_id = 1;
    SLayerId m_next_layer_id = 1;
    QString m_file_path;
    std::vector<SLayerRecord> m_layers{
        {0, QStringLiteral("0"), QColor(220, 228, 238), 0.25, true, false, true},
    };
    std::vector<SLayerStateRecord> m_layer_states;
    QString m_current_layer_name = QStringLiteral("0");
    QColor m_preferred_drawing_color;
    SDrawingSettings m_drawing_settings;
    std::vector<STextStyleRecord> m_text_styles{{}};
    QString m_current_text_style_name = QStringLiteral("Standard");
    std::vector<SDimensionStyleRecord> m_dimension_styles{{}};
