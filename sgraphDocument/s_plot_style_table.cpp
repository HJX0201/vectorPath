...
void parseStyleProperty(SParsedStyle& style, const QString& line)
{
...
SResult<SPlotStyleTable> parsePlotStyleData(const QByteArray& data, const QString& file_path)
{
    ...
    bool in_style = false;
    SParsedStyle current_style;
    std::vector<SParsedStyle> parsed_styles;
    ...
    {
        return SResult<SPlotStyleTable>::failure(
            QObject::tr("打印样式表不包含可用样式：%1").arg(file_path));
    }
    SPlotStyleTable table;
    table.m_type =
        is_color_dependent ? SPlotStyleTableType::ColorDependent : SPlotStyleTableType::Named;
    table.m_description = description;
    table.m_styles.reserve(parsed_styles.size());
    for (SParsedStyle& parsed_style : parsed_styles)
    {
...
SResult<SPlotStyleTable> SPlotStyleTable::load(const QString& file_path)
{
    QFile file(file_path);
    if (!file.open(QIODevice::ReadOnly))
    {
        return SResult<SPlotStyleTable>::failure(
            QObject::tr("无法读取打印样式表：%1").arg(file.errorString()));
    }
    const QByteArray data = decompressedPlotStyleData(file.readAll());
    if (data.isEmpty())
    {
        return SResult<SPlotStyleTable>::failure(
            QObject::tr("打印样式表头或压缩数据无效：%1").arg(file_path));
    }
    return parsePlotStyleData(data, file_path);
}

SResult<SPlotStyleTable> SPlotStyleTable::fromReference(const QString& reference)
{
    if (reference.compare(QLatin1String("monochrome.ctb"), Qt::CaseInsensitive) == 0)
    {
        return SResult<SPlotStyleTable>::success(monochromeTable());
    }
...
SPlotStyleTableType SPlotStyleTable::type() const noexcept
{
    return m_type;
}

QString SPlotStyleTable::description() const
{
    return m_description;
}

const std::vector<SPlotStyleRecord>& SPlotStyleTable::styles() const noexcept
{
    return m_styles;
}

const SPlotStyleRecord* SPlotStyleTable::styleForColor(const QColor& source_color) const noexcept
{
    if (m_styles.empty())
    {
        return nullptr;
    }
    if (m_type == SPlotStyleTableType::Named)
    {
        const auto iterator = std::find_if(m_styles.begin(), m_styles.end(),
                                           [](const SPlotStyleRecord& style)
                                           {
    ...
    const auto iterator = std::find_if(m_styles.begin(), m_styles.end(),
                                       [style_index](const SPlotStyleRecord& style)
                                       {
...
QColor SPlotStyleTable::mappedColor(const QColor& source_color) const
{
    QColor result = source_color;
    const SPlotStyleRecord* style = styleForColor(source_color);
...
double SPlotStyleTable::mappedLineWidth(const QColor& source_color,
...
SPlotStyleTable SPlotStyleTable::monochromeTable()
{
    SPlotStyleTable table;
...
SPlotStyleTable SPlotStyleTable::grayscaleTable()
{
    SPlotStyleTable table;
