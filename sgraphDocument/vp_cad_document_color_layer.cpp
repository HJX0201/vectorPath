...
QString uniqueColorLayerName(const VpCadDocument& document, const QColor& color)
{
...
void VpCadDocument::setPreferredDrawingColor(const QColor& color)
{
    m_preferred_drawing_color = color.isValid() ? color : QColor{};
}

QColor VpCadDocument::preferredDrawingColor() const
{
    return m_preferred_drawing_color;
}

QString VpCadDocument::resolveDrawingLayerName()
{
    if (!m_preferred_drawing_color.isValid())
    {
        return m_current_layer_name;
    }
    const VpLayerRecord* current_layer = layer(m_current_layer_name);
    ...
    const auto matching_layer =
        std::find_if(m_layers.begin(), m_layers.end(),
                     [this](const VpLayerRecord& layer_record)
                     {
...
QString VpCadDocument::assignEntitiesToColorLayer(const std::vector<VpEntityId>& entity_ids,
                                                  const QColor& color)
{
    ...
    QSet<VpEntityId> moved_ids;
    const VpLayerRecord* template_layer = nullptr;
    ...
    VpLayerRecord new_layer = template_layer ? *template_layer : VpLayerRecord{};
    ...
    for (const VpEntityRecord& entity : removed_entities)
    {
    ...
    for (const QString& source_layer_name : source_layer_names)
    {
        ...
        const bool has_unselected_entity = std::any_of(
            m_entities.begin(), m_entities.end(),
            [&moved_ids, &source_layer_name](const VpEntityRecord& entity)
            {
                return !moved_ids.contains(entity.id) &&
                       entity.layer_name.compare(source_layer_name, Qt::CaseInsensitive) == 0;
            });
        ...
        m_layers.erase(std::remove_if(m_layers.begin(), m_layers.end(),
                                      [&source_layer_name](const VpLayerRecord& layer_record)
                                      {
                                          return layer_record.name.compare(
                                                     source_layer_name, Qt::CaseInsensitive) == 0;
                                      }),
                       m_layers.end());
    }
...
int VpCadDocument::mergeLayersByColor()
{
    ...
    m_layers.erase(std::remove_if(m_layers.begin(), m_layers.end(),
                                  [&source_to_target](const VpLayerRecord& layer_record)
                                  {
