...
const std::vector<VpDimensionStyleRecord>& VpCadDocument::dimensionStyles() const noexcept
{
    return m_dimension_styles;
}

const VpDimensionStyleRecord*
VpCadDocument::dimensionStyle(const QString& style_name) const noexcept
{
    const auto iterator =
...
QString VpCadDocument::currentDimensionStyleName() const
{
    return m_current_dimension_style_name;
}

bool VpCadDocument::addOrUpdateDimensionStyle(const VpDimensionStyleRecord& dimension_style)
{
    VpDimensionStyleRecord normalized = dimension_style;
    normalized.name = normalized.name.simplified();
    if (!validDimensionStyle(normalized))
    {
        return false;
    }
    const std::vector<VpDimensionStyleRecord> styles_before = m_dimension_styles;
    const QString current_style_before = m_current_dimension_style_name;
    auto iterator =
        std::find_if(m_dimension_styles.begin(), m_dimension_styles.end(),
                     [&normalized](const VpDimensionStyleRecord& style)
                     {
...
bool VpCadDocument::removeDimensionStyle(const QString& style_name)
{
    const auto iterator =
        std::find_if(m_dimension_styles.begin(), m_dimension_styles.end(),
                     [&style_name](const VpDimensionStyleRecord& style)
        ...
        std::any_of(m_entities.begin(), m_entities.end(),
                    [&style_name](const VpEntityRecord& entity)
                    {
                        return entity.type == VpEntityType::LinearDimension &&
                               std::get<VpLinearDimensionEntity>(entity.geometry)
...
bool VpCadDocument::setCurrentDimensionStyle(const QString& style_name)
{
    const VpDimensionStyleRecord* style = dimensionStyle(style_name);
...
void VpCadDocument::commitDimensionStyleChange(QString label,
