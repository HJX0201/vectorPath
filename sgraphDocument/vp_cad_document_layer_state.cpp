...
QStringList VpCadDocument::layerStateNames() const
{
...
bool VpCadDocument::saveLayerState(const QString& state_name)
{
    ...
    auto iterator =
        std::find_if(m_layer_states.begin(), m_layer_states.end(),
                     [&](const VpLayerStateRecord& state)
                     {
                         return state.name.compare(normalized_name, Qt::CaseInsensitive) == 0;
                     });
    VpLayerStateRecord snapshot{normalized_name, m_layers, m_current_layer_name};
...
bool VpCadDocument::restoreLayerState(const QString& state_name)
{
    ...
    for (VpLayerRecord& layer_record : m_layers)
    {
        ...
        VpLayerRecord restored = *snapshot_iterator;
    ...
    const VpLayerRecord* restored_current = layer(state_iterator->current_layer_name);
...
bool VpCadDocument::removeLayerState(const QString& state_name)
{
    const auto iterator =
        std::find_if(m_layer_states.begin(), m_layer_states.end(),
                     [&](const VpLayerStateRecord& state)
