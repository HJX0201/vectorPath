...
{
    const SDocumentAuditReport report = m_document->audit(repair);
    ...
    {
        const SDocumentAuditIssue& issue = report.issues[static_cast<std::size_t>(index)];
    ...
    if (report.isClean())
    {
        SDialogService::information(this, tr("图形审计"),
                                    tr("未发现图形数据库问题。\n%1").arg(summary));
    }
    else
    {
        SDialogService::warning(this, tr("图形审计"), summary);
    }
}
