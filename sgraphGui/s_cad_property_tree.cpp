...
QString entityTypeName(SEntityType entity_type)
{
    switch (entity_type)
    {
...
double polygonArea(const std::vector<SPoint2d>& vertices)
{
    if (vertices.size() < 3)
    {
        return 0.0;
    ...
    {
        const SPoint2d& current = vertices[index];
        const SPoint2d& next = vertices[(index + 1) % vertices.size()];
                                       ...
                                       const SCadDocument& document, const SDesignToken& tokens)
{
    ...
    const SLayerRecord* layer_record = document.layer(entity.layer_name);
    ...
    {
        const SAssociativeArrayData& array_data = *entity.associative_array;
        ...
        const QString type_name =
            array_data.array_type == SArrayType::Rectangular
                ? QObject::tr("矩形")
                : (array_data.array_type == SArrayType::Polar ? QObject::tr("环形")
                                                              : QObject::tr("路径"));
        ...
        if (array_data.array_type == SArrayType::Rectangular)
        {
            addProperty(
        ...
        else if (array_data.array_type == SArrayType::Polar)
        {
    ...
    else if (entity.type == SEntityType::Hatch)
    {
        ...
        if (hatch.fill_type == SHatchFillType::Pattern)
        {
