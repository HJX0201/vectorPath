...
void drawRegularPolygon(QPainter& painter, int vertex_count, double inner_ratio = 1.0)
{
    ...
    for (int index = 0; index < vertex_count; ++index)
    {
        ...
        polygon << QPointF(32.0 + std::cos(angle) * radius,
                           32.0 + std::sin(angle) * radius);
    }
    painter.drawPolygon(polygon);
}

void drawBooleanOperands(QPainter& painter, SIconType icon_type,
                         const QColor& foreground, const QColor& accent_color)
{
    ...
    if (icon_type == SIconType::BooleanUnion)
    {
        result_path = left_path.united(right_path);
    }
    else if (icon_type == SIconType::BooleanIntersection)
    {
        result_path = left_path.intersected(right_path);
    }
    else if (icon_type == SIconType::BooleanDifference)
    {
        result_path = left_path.subtracted(right_path);
    }
    else if (icon_type == SIconType::BooleanComplement)
    {
...
void drawShapeBooleanIcon(QPainter& painter, SIconType icon_type,
                          const QColor& foreground, const QColor& accent_color)
{
    if (icon_type == SIconType::ShapeStar)
    {
        drawRegularPolygon(painter, 10, 0.3819660112501051);
    }
    else if (icon_type == SIconType::ShapeTriangle)
    {
        drawRegularPolygon(painter, 3);
    }
    else if (icon_type == SIconType::ShapePentagon)
    {
        drawRegularPolygon(painter, 5);
    }
    else if (icon_type == SIconType::ShapeHexagon)
    {
        drawRegularPolygon(painter, 6);
    }
    else if (icon_type == SIconType::ShapeOctagon)
    {
        drawRegularPolygon(painter, 8);
    }
    else if (icon_type == SIconType::ShapeDiamond)
    {
