...
SThemeManager::SThemeManager(QObject* parent) : QObject(parent)
{
    QSettings settings;
    const int saved_scale =
        settings.value(QStringLiteral("ui/scalePercent"), 100).toInt();
    m_ui_scale_percent = std::max(100, std::min(200, saved_scale));
}

SThemeMode SThemeManager::themeMode() const noexcept
{
    return m_theme_mode;
}

const SDesignToken& SThemeManager::tokens() const noexcept
{
    return m_tokens;
}

int SThemeManager::uiScalePercent() const noexcept
{
    return m_ui_scale_percent;
}

bool SThemeManager::setUiScalePercent(int scale_percent)
{
...
bool SThemeManager::applyTheme(SThemeMode theme_mode)
{
    QString resource_path;
    switch (theme_mode)
    {
    case SThemeMode::Light:
        resource_path = QStringLiteral(":/smartcad/themes/s_theme_light.json");
        break;
    case SThemeMode::Dark:
        resource_path = QStringLiteral(":/smartcad/themes/s_theme_dark.json");
        break;
    case SThemeMode::HighContrast:
        resource_path = QStringLiteral(":/smartcad/themes/s_theme_high_contrast.json");
        break;
    }

    SDesignToken new_tokens;
...
bool SThemeManager::loadTokens(const QString& resource_path, SDesignToken& tokens) const
{
...
void SThemeManager::applyPaletteAndStyle()
{
    ...
    if (dynamic_cast<SProxyStyle*>(application->style()) == nullptr)
    {
        application->setStyle(new SProxyStyle());
    }
