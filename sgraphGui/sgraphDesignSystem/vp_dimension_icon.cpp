...
void drawDimensionIcon(QPainter& painter, VpIconType icon_type, const QColor& accent_color)
{
    ...
    if (icon_type == VpIconType::Dimension || icon_type == VpIconType::DimensionAligned)
    {
        const bool aligned = icon_type == VpIconType::DimensionAligned;
    ...
    else if (icon_type == VpIconType::DimensionAngular ||
             icon_type == VpIconType::DimensionArcLength)
    {
        painter.setBrush(Qt::NoBrush);
        ...
        if (icon_type == VpIconType::DimensionArcLength)
        {
            painter.setPen(QPen(accent_color, 3.0));
            painter.drawArc(QRectF(13, 8, 38, 20), 15 * 16, 150 * 16);
        }
    }
    else if (icon_type == VpIconType::DimensionRadius || icon_type == VpIconType::DimensionDiameter)
    {
        painter.setBrush(Qt::NoBrush);
        painter.drawEllipse(QRectF(11, 11, 42, 42));
        painter.drawLine(icon_type == VpIconType::DimensionDiameter ? QPointF(16, 48)
                                                                    : QPointF(32, 32),
                         QPointF(48, 16));
        painter.setBrush(accent_color);
        drawArrow(painter, QPointF(48, 16), QPointF(-7, 7));
        painter.drawText(QPointF(5, 61), icon_type == VpIconType::DimensionDiameter
                                             ? QString(QChar(0x2300))
                                             : QStringLiteral("R"));
    }
    else if (icon_type == VpIconType::DimensionOrdinate)
    {
        painter.setBrush(Qt::NoBrush);
    ...
    else if (icon_type == VpIconType::DimensionStyle)
    {
        painter.setBrush(Qt::NoBrush);
