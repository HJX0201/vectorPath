...
namespace Vp
{
VpCadMainWindow::VpCadMainWindow(VpThemeManager& theme_manager, QWidget* parent)
    : SARibbonMainWindow(parent, SARibbonMainWindowStyleFlag::UseRibbonMenuBar |
                                     SARibbonMainWindowStyleFlag::UseNativeFrame),
      m_theme_manager(theme_manager), m_shortcut_manager(std::make_unique<VpShortcutManager>()),
      m_document(std::make_unique<VpCadDocument>())
{
    ...
    m_simulation_controller = std::make_unique<VpToolpathSimulationController>(
        m_document.get(), m_workspace->viewport(), this);
    createStatusBarWidgets();
    connectDocument();
    m_recovery_manager = std::make_unique<VpDocumentRecoveryManager>(m_document.get(), this);
    connect(m_recovery_manager.get(), &VpDocumentRecoveryManager::recoveryFailed, this,
            [this](const QString& error_message)
            {
                m_command_line->appendMessage(tr("自动保存失败：%1").arg(error_message));
            });
    m_recovery_manager->start();
    connect(&m_theme_manager, &VpThemeManager::themeChanged, this,
            [this](VpThemeMode)
            {
                refreshIcons();
            });
    connect(&m_theme_manager, &VpThemeManager::uiScaleChanged, this,
            &VpCadMainWindow::applyUiScale);
    restoreWorkspace();
...
VpCadMainWindow::~VpCadMainWindow()
{
    if (m_workspace && m_workspace->viewport())
    {
        QObject::disconnect(m_workspace->viewport(), &VpCadViewport::toolModeChanged, this,
                            &VpCadMainWindow::updateToolButtonHighlight);
    }
    m_tool_actions.clear();
    m_icon_actions.clear();
}

void VpCadMainWindow::closeEvent(QCloseEvent* event)
{
    if (!maybeSave())
    {
        event->ignore();
...
void VpCadMainWindow::createRibbon()
{
    ...
    svg_panel->addLargeAction(m_svg_deduplicate_action, QToolButton::MenuButtonPopup);
    QAction* polyline_action = createAction(tr("多段线"), VpIconType::DrawPolyline);
    QAction* rectangle_action = createAction(tr("矩形"), VpIconType::DrawRectangle);
    ...
    registerToolAction(polyline_action, VpToolMode::Polyline);
    registerToolAction(rectangle_action, VpToolMode::Rectangle);
    connect(polyline_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Polyline);
            });
    connect(rectangle_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Rectangle);
            });
    ...
    const struct VpShapeActionSpec
    {
        const char* text;
        VpIconType icon_type;
        VpStandardShapeType shape_type;
        const char* command_id;
    } shape_specs[]{
        {"五角星", VpIconType::ShapeStar, VpStandardShapeType::FivePointStar, "draw.star"},
        {"三角形", VpIconType::ShapeTriangle, VpStandardShapeType::Triangle, "draw.triangle"},
        {"五边形", VpIconType::ShapePentagon, VpStandardShapeType::Pentagon, "draw.pentagon"},
        {"六边形", VpIconType::ShapeHexagon, VpStandardShapeType::Hexagon, "draw.hexagon"},
        {"八边形", VpIconType::ShapeOctagon, VpStandardShapeType::Octagon, "draw.octagon"},
        {"菱形", VpIconType::ShapeDiamond, VpStandardShapeType::Diamond, "draw.diamond"},
    };
    for (const VpShapeActionSpec& shape_spec : shape_specs)
    {
        ...
        registerToolAction(shape_action, VpToolMode::StandardShape);
    ...
    QAction* move_action = createAction(tr("移动"), VpIconType::Move);
    QAction* copy_action = createAction(tr("复制"), VpIconType::Copy);
    QAction* rotate_action = createAction(tr("旋转"), VpIconType::Rotate);
    QAction* scale_action = createAction(tr("缩放"), VpIconType::Scale);
    QAction* mirror_action = createAction(tr("镜像"), VpIconType::Mirror);
    QAction* trim_action = createAction(tr("修剪"), VpIconType::Trim);
    QAction* extend_action = createAction(tr("延伸"), VpIconType::Extend);
    QAction* break_action = createAction(tr("打断"), VpIconType::Break);
    QAction* join_action = createAction(tr("合并"), VpIconType::Join);
    QAction* explode_action = createAction(tr("分解"), VpIconType::Explode);
    QAction* stretch_action = createAction(tr("拉伸"), VpIconType::Stretch);
    QAction* lengthen_action = createAction(tr("拉长"), VpIconType::Lengthen);
    QAction* polyline_edit_action = createAction(tr("编辑多段线"), VpIconType::PolylineEdit);
    QAction* spline_edit_action = createAction(tr("编辑样条"), VpIconType::SplineEdit);
    QAction* grip_action = createAction(tr("夹点编辑"), VpIconType::Grip);
    grip_action->setObjectName(QStringLiteral("smartGripAction"));
    QAction* fillet_action = createAction(tr("圆角"), VpIconType::Fillet);
    QAction* chamfer_action = createAction(tr("倒角"), VpIconType::Chamfer);
    QAction* blend_action = createAction(tr("混接曲线"), VpIconType::Blend);
    QAction* align_action = createAction(tr("对齐"), VpIconType::Align);
    QAction* array_rect_action = createAction(tr("矩形阵列"), VpIconType::ArrayRect);
    QAction* array_polar_action = createAction(tr("环形阵列"), VpIconType::ArrayPolar);
    QAction* array_path_action = createAction(tr("路径阵列"), VpIconType::ArrayPath);
    QAction* array_edit_action = createAction(tr("编辑阵列"), VpIconType::ArrayEdit);
    QAction* offset_action = createAction(tr("偏移"), VpIconType::Offset);
    ...
    registerToolAction(move_action, VpToolMode::Move);
    registerToolAction(copy_action, VpToolMode::Copy);
    registerToolAction(rotate_action, VpToolMode::Rotate);
    registerToolAction(scale_action, VpToolMode::Scale);
    registerToolAction(mirror_action, VpToolMode::Mirror);
    registerToolAction(offset_action, VpToolMode::Offset);
    registerToolAction(trim_action, VpToolMode::Trim);
    registerToolAction(extend_action, VpToolMode::Extend);
    registerToolAction(break_action, VpToolMode::Break);
    registerToolAction(join_action, VpToolMode::Join);
    registerToolAction(explode_action, VpToolMode::Explode);
    registerToolAction(stretch_action, VpToolMode::Stretch);
    registerToolAction(lengthen_action, VpToolMode::Lengthen);
    registerToolAction(polyline_edit_action, VpToolMode::PolylineEdit);
    registerToolAction(spline_edit_action, VpToolMode::SplineEdit);
    registerToolAction(fillet_action, VpToolMode::Fillet);
    registerToolAction(chamfer_action, VpToolMode::Chamfer);
    registerToolAction(blend_action, VpToolMode::Blend);
    registerToolAction(align_action, VpToolMode::Align);
    registerToolAction(array_rect_action, VpToolMode::ArrayRect);
    registerToolAction(array_polar_action, VpToolMode::ArrayPolar);
    registerToolAction(array_path_action, VpToolMode::ArrayPath);
    registerToolAction(array_edit_action, VpToolMode::ArrayEdit);
    connect(move_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Move);
            });
    connect(copy_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Copy);
            });
    connect(rotate_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Rotate);
            });
    connect(scale_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Scale);
            });
    connect(mirror_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Mirror);
            });
    connect(offset_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Offset);
            });
    connect(trim_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Trim);
            });
    connect(extend_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Extend);
            });
    connect(break_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Break);
            });
    connect(join_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Join);
            });
    connect(explode_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Explode);
            });
    connect(stretch_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Stretch);
            });
    connect(lengthen_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Lengthen);
            });
    connect(polyline_edit_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::PolylineEdit);
            });
    connect(spline_edit_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::SplineEdit);
            });
    configureGripAction(grip_action);
    connect(fillet_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Fillet);
            });
    connect(chamfer_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Chamfer);
            });
    connect(blend_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Blend);
            });
    connect(align_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Align);
            });
    connect(array_rect_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::ArrayRect);
            });
    connect(array_polar_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::ArrayPolar);
            });
    connect(array_path_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::ArrayPath);
            });
    connect(array_edit_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::ArrayEdit);
            });
    ...
    QAction* dimension_action = createAction(tr("线性标注"), VpIconType::Dimension);
    QAction* text_action = createAction(tr("文字"), VpIconType::Text);
    QAction* hatch_action = createAction(tr("填充"), VpIconType::Hatch);
    ...
    registerToolAction(dimension_action, VpToolMode::LinearDimension);
    registerToolAction(text_action, VpToolMode::Text);
    registerToolAction(hatch_action, VpToolMode::Hatch);
    connect(dimension_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::LinearDimension);
            });
    connect(text_action, &QAction::triggered, this, &VpCadMainWindow::beginTextCommand);
    connect(hatch_action, &QAction::triggered, this,
            [this]()
            {
                m_workspace->viewport()->setToolMode(VpToolMode::Hatch);
            });
    ...
    QAction* recovery_action = createAction(tr("图形恢复"), VpIconType::Recovery);
    QAction* recovery_copy_action = createAction(tr("立即自动保存"), VpIconType::Save);
    QAction* audit_action = createAction(tr("审计并修复"), VpIconType::Audit);
    ...
    connect(recovery_action, &QAction::triggered, this, &VpCadMainWindow::showRecoveryManager);
    connect(recovery_copy_action, &QAction::triggered, this, &VpCadMainWindow::createRecoveryCopy);
    ...
    QAction* settings_action = createAction(tr("界面比例"), VpIconType::UiScale);
    QAction* shortcut_action = createAction(tr("修改快捷键"), VpIconType::Settings);
    shortcut_action->setObjectName(QStringLiteral("smartModifyShortcutsAction"));
    connect(settings_action, &QAction::triggered, this, &VpCadMainWindow::showInterfaceSettings);
    registerShortcutAction(shortcut_action, QStringLiteral("ui.shortcuts"));
    connect(shortcut_action, &QAction::triggered, this, &VpCadMainWindow::showShortcutSettings);
    ...
    QAction* help_action =
        createAction(tr("使用帮助"), VpIconType::Help, QKeySequence::HelpContents);
...
void VpCadMainWindow::registerToolAction(QAction* action, VpToolMode tool_mode)
{
    action->setCheckable(true);
    m_tool_actions.emplace_back(static_cast<std::uint8_t>(tool_mode), action);
}

void VpCadMainWindow::updateToolButtonHighlight(VpToolMode current_mode)
{
