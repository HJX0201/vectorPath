...
void VpCadMainWindow::configureAnnotationPanel(SARibbonPanel* annotation_panel)
{
    QAction* mtext_action = createAction(tr("多行文字"), VpIconType::MText);
    QAction* leader_action = createAction(tr("多重引线"), VpIconType::Leader);
    QAction* style_action = createAction(tr("文字样式"), VpIconType::TextStyle);
    ...
    connect(style_action, &QAction::triggered, this, &VpCadMainWindow::showTextStyleManager);
...
void VpCadMainWindow::beginMTextCommand(const QString& initial_text)
{
    ...
    alignment_combo->addItem(tr("左对齐"), static_cast<int>(VpTextHorizontalAlignment::Left));
    alignment_combo->addItem(tr("居中"), static_cast<int>(VpTextHorizontalAlignment::Center));
    alignment_combo->addItem(tr("右对齐"), static_cast<int>(VpTextHorizontalAlignment::Right));
    alignment_combo->addItem(tr("两端对齐"),
                             static_cast<int>(VpTextHorizontalAlignment::Justified));
    ...
    m_workspace->viewport()->setPendingMText(
        ...
        static_cast<VpTextHorizontalAlignment>(alignment_combo->currentData().toInt()));
    m_workspace->viewport()->setToolMode(VpToolMode::MText);
}

void VpCadMainWindow::beginLeaderCommand(const QString& initial_text)
{
    VpDialog dialog(this);
    ...
    m_workspace->viewport()->setPendingLeader(text_edit->text(), height_box->value(),
                                              arrow_box->value());
    m_workspace->viewport()->setToolMode(VpToolMode::Leader);
}

bool VpCadMainWindow::executeAnnotationCommand(const QString& command)
{
    ...
    if (normalized.startsWith(QLatin1String("TEXTALIGN ")))
    {
        ...
        {
            alignment = VpTextHorizontalAlignment::Center;
        }
        else if (alignment_name == QLatin1String("RIGHT") || alignment_name == QLatin1String("R"))
        {
            alignment = VpTextHorizontalAlignment::Right;
        }
