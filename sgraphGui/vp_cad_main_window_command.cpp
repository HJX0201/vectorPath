...
bool VpCadMainWindow::executeLayerCommand(const QString& simplified_command)
{
    const QStringList parts = simplified_command.split(QLatin1Char(' '), QString::SkipEmptyParts);
    if (parts.isEmpty() || parts[0].compare(QLatin1String("LAYER"), Qt::CaseInsensitive) != 0)
    {
        return false;
    }
    ...
    if (operation == QLatin1String("MERGECOLOR") && parts.size() == 2)
    {
        ...
        m_command_line->appendMessage(merged_count > 0
    ...
    else if (operation == QLatin1String("DISPLAY") && parts.size() == 4)
    {
        ...
        if (display_state == QLatin1String("ON") || display_state == QLatin1String("OFF"))
        {
            ...
            is_success = layer_record && (layer_record->is_visible == is_visible ||
    ...
    else if (operation == QLatin1String("COLOR") && parts.size() == 4)
    {
        ...
        {
            const VpLayerRecord* layer_record = m_document->layer(layer_name);
            is_success = layer_record && (layer_record->color == color ||
    ...
    if (!is_success)
    {
        m_command_line->appendMessage(tr("图层操作失败。可用命令：LAYER DELETE|CLEAR 图层，"
...
bool VpCadMainWindow::executeArrayParameterCommand(const QString& normalized_command)
{
