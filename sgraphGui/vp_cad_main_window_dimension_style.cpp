...
void VpCadMainWindow::showDimensionStyleManager()
{
                             ...
                             current_label]()
    {
        const VpDimensionStyleRecord* style =
            m_document->dimensionStyle(style_combo->currentText());
        if (!style)
        {
            return;
        }
    ...
    const auto refresh = [this, style_combo, load_style]()
    {
        ...
        style_combo->clear();
        for (const VpDimensionStyleRecord& style : m_document->dimensionStyles())
        {
    ...
    connect(save_button, &QPushButton::clicked, &dialog,
             ...
             suppress_zeros_check, refresh]()
            {
                ...
                {
                    VpDialogService::warning(this, tr("标注样式"), tr("样式参数无效。"));
                    return;
                }
                refresh();
            });
...
bool VpCadMainWindow::executeDimensionStyleCommand(const QString& command)
{
    ...
    if (is_override_command)
    {
        ...
        for (quint64 selected_id : selected_ids)
        {
            ...
            if (iterator == m_document->entities().end() ||
                iterator->type != VpEntityType::LinearDimension)
            {
                continue;
            }
            VpEntityRecord replacement = *iterator;
            auto& dimension = std::get<VpLinearDimensionEntity>(replacement.geometry);
            if (parts.size() == 3 &&
