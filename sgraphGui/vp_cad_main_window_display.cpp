...
void VpCadMainWindow::configureViewOptions(SARibbonCategory* view_category)
{
    SARibbonPanel* precision_panel = view_category->addPanel(tr("精确绘图"));
    QAction* coordinate_action = createAction(tr("精确坐标"), VpIconType::CommandLine,
    ...
    m_snap_action = createAction(tr("捕捉"), VpIconType::Snap);
    ...
    QAction* units_action = createAction(tr("图形单位"), VpIconType::Settings);
    units_action->setToolTip(tr("设置插入单位、角度格式和显示精度"));
    registerShortcutAction(units_action, QStringLiteral("precision.units"));
    connect(units_action, &QAction::triggered, this, &VpCadMainWindow::showUnitsDialog);
    ...
    QAction* display_action = createAction(tr("显示"), VpIconType::Viewport);
    ...
    connect(&m_theme_manager, &VpThemeManager::themeChanged, this,
            [refresh_check_icons](VpThemeMode)
            {
                refresh_check_icons();
            });
