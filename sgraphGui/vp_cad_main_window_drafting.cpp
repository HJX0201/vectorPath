...
std::optional<VpStandardShapeType> standardShapeFromCommand(const QString& command)
{
    if (command == QLatin1String("STAR"))
    {
        return VpStandardShapeType::FivePointStar;
    }
    if (command == QLatin1String("TRIANGLE"))
    {
        return VpStandardShapeType::Triangle;
    }
    if (command == QLatin1String("PENTAGON"))
    {
        return VpStandardShapeType::Pentagon;
    }
    if (command == QLatin1String("HEXAGON"))
    {
        return VpStandardShapeType::Hexagon;
    }
    if (command == QLatin1String("OCTAGON"))
    {
        return VpStandardShapeType::Octagon;
    }
    if (command == QLatin1String("DIAMOND"))
    {
        return VpStandardShapeType::Diamond;
    }
    return std::nullopt;
}
...
bool VpCadMainWindow::executeDraftingCommand(const QString& normalized_command)
{
    VpCadViewport* viewport = m_workspace->viewport();
    const std::optional<VpStandardShapeType> standard_shape =
        ...
        normalized_command.startsWith(QLatin1String("ARC SER")))
    {
        ...
        VpArcConstruction construction = VpArcConstruction::StartCenterAngle;
    ...
    VpDrawingSettings drawing_settings = m_document->drawingSettings();
    bool has_drawing_settings_command = false;
    bool is_drawing_settings_valid = true;
    if (parts.size() == 2 && parts[0] == QLatin1String("UNITS"))
    {
        const std::optional<VpInsertionUnit> insertion_unit = insertionUnitFromKey(parts[1]);
    ...
    {
        const std::optional<VpAngleFormat> angle_format = angleFormatFromKey(parts[1]);
