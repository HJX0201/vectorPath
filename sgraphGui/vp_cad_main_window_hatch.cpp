...
bool parseHatchCommand(const QStringList& parts, int option_index, VpHatchEntity& settings)
{
    ...
    {
        settings.fill_type = VpHatchFillType::Solid;
    ...
    if (option == QLatin1String("PATTERN") && parts.size() >= option_index + 2)
    {
        ...
        settings.fill_type = VpHatchFillType::Pattern;
    ...
    if (option == QLatin1String("GRADIENT") && parts.size() >= option_index + 3)
    {
        ...
        settings.fill_type = VpHatchFillType::Gradient;
...
void VpCadMainWindow::configureHatchPanel(SARibbonPanel* annotation_panel)
{
    QAction* solid_action = createAction(tr("实体填充"), VpIconType::Hatch);
    QAction* pattern_action = createAction(tr("图案填充"), VpIconType::HatchPattern);
    QAction* gradient_action = createAction(tr("渐变填充"), VpIconType::HatchGradient);
    QAction* edit_action = createAction(tr("编辑填充"), VpIconType::HatchEdit);
    ...
    connect(solid_action, &QAction::triggered, this,
            [this]()
            {
                VpHatchEntity settings = m_workspace->viewport()->hatchSettings();
                settings.fill_type = VpHatchFillType::Solid;
...
void VpCadMainWindow::showHatchSettings(bool edit_selected)
{
    ...
    fill_type_combo->addItem(tr("实体"), static_cast<int>(VpHatchFillType::Solid));
    fill_type_combo->addItem(tr("图案"), static_cast<int>(VpHatchFillType::Pattern));
    fill_type_combo->addItem(tr("渐变"), static_cast<int>(VpHatchFillType::Gradient));
    ...
    settings.fill_type = static_cast<VpHatchFillType>(fill_type_combo->currentData().toInt());
...
bool VpCadMainWindow::executeHatchCommand(const QString& command)
{
