...
QWidget* centeredCellWidget(QWidget* content, QWidget* parent)
{
    ...
    container->setAttribute(Qt::WA_TransparentForMouseEvents, content->inherits("QFrame"));
...
bool VpCadMainWindow::clearLayerEntities(const QString& layer_name)
{
...
ads::CDockWidget* VpCadMainWindow::createLayerDock()
{
    ...
    layer_dock->setMinimumSizeHintMode(ads::CDockWidget::MinimumSizeHintFromContentMinimumSize);
    layer_dock->setIcon(
        VpIconProvider::createIcon(VpIconType::Layers, tokens.text_primary, tokens.accent));
    m_icon_docks.emplace_back(layer_dock, VpIconType::Layers);
    ...
    merge_color_button->setIcon(
        VpIconProvider::createIcon(VpIconType::BooleanUnion, tokens.text_primary, tokens.accent));
    ...
    const auto refresh_layer_table = [this, layer_table]()
    {
        ...
        for (const VpLayerRecord& layer_record : m_document->layers())
        {
            ...
            const int entity_count = static_cast<int>(std::count_if(
                m_document->entities().begin(), m_document->entities().end(),
                [&layer_record](const VpEntityRecord& entity)
                {
                    return entity.layer_name.compare(layer_record.name, Qt::CaseInsensitive) == 0;
                }));
            ...
            layer_table->setCellWidget(row, 2, centeredCellWidget(color_block, layer_table));
            ...
            layer_table->setCellWidget(row, 4, centeredCellWidget(visible_check, layer_table));
    ...
    refresh_layer_table();
    connect(m_document.get(), &VpCadDocument::layersChanged, layer_table, refresh_layer_table);
    connect(m_document.get(), &VpCadDocument::documentChanged, layer_table, refresh_layer_table);

    connect(
        delete_layer_button, &QPushButton::clicked, this,
        [this, layer_table]()
        {
    ...
    connect(clear_entities_button, &QPushButton::clicked, this,
            [this, layer_table]()
            {
                ...
                if (VpDialogService::question(this, tr("删除图层实体"),
    ...
    connect(merge_color_button, &QPushButton::clicked, this,
            [this]()
            {
                ...
                if (merged_count == 0)
                {
                    VpDialogService::information(this, tr("合并同色图层"),
                                                 tr("没有可合并的同色普通图层。"));
                    return;
                }
    ...
    connect(layer_table, &QTableWidget::cellClicked, this,
            [this, layer_table](int row, int column)
            {
                ...
                const QString layer_name = layer_item->data(Qt::UserRole).toString();
                const VpLayerRecord* layer_record = m_document->layer(layer_name);
                if (!layer_record)
                {
                    return;
                }
                if (column == 2)
                {
                    ...
                    const QColor color = color_dialog.exec() == QColorDialog::Accepted
