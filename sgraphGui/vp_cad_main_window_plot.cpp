...
VpResult<void> installPlotStyleFile(const QString& source_path)
{
    const VpResult<VpPlotStyleTable> validation = VpPlotStyleTable::load(source_path);
    if (!validation)
    {
        return VpResult<void>::failure(toCoreText(toQtError(validation)));
    }
    const QString directory_path = plotStyleDirectoryPath();
    if (!QDir().mkpath(directory_path))
    {
        return VpResult<void>::failure(toCoreText(QObject::tr("无法创建打印样式目录。")));
    }
    QFile source_file(source_path);
    if (!source_file.open(QIODevice::ReadOnly))
    {
        return VpResult<void>::failure(toCoreText(source_file.errorString()));
    }
        ...
        !target_file.commit())
    {
        return VpResult<void>::failure(
            toCoreText(QObject::tr("无法安装打印样式表：%1").arg(target_file.errorString())));
    }
...
bool VpCadMainWindow::executeOutputCommand(const QString& simplified_command)
{
...
void VpCadMainWindow::showPlotStyleManager()
{
    ...
    connect(
        list_widget, &QListWidget::currentItemChanged, &dialog,
        [details_label, remove_button](QListWidgetItem* current)
        {
            ...
            const VpResult<VpPlotStyleTable> result = VpPlotStyleTable::fromReference(reference);
            remove_button->setEnabled(current->data(Qt::UserRole + 1).toBool());
            if (!result)
            {
                details_label->setText(tr("读取失败：%1").arg(toQtError(result)));
                return;
            }
            const VpPlotStyleTable& table = result.value();
            details_label->setText(
                tr("类型：%1\n样式数量：%2\n说明：%3")
                    .arg(table.type() == VpPlotStyleTableType::ColorDependent ? tr("颜色相关 CTB")
                                                                              : tr("命名样式 STB"))
    ...
    connect(import_button, &QPushButton::clicked, &dialog,
            [this, &dialog, refresh_list]()
            {
                ...
                const VpResult<void> result = installPlotStyleFile(file_path);
                if (!result)
                {
                    VpDialogService::warning(&dialog, tr("打印样式管理器"), toQtError(result));
                    return;
                }
                refresh_list();
    ...
    connect(remove_button, &QPushButton::clicked, &dialog,
            [list_widget, refresh_list, &dialog]()
            {
                ...
                {
                    VpDialogService::warning(&dialog, tr("打印样式管理器"),
                                             tr("无法删除所选打印样式表。"));
                }
            });
