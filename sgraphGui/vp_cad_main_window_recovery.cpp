...
void VpCadMainWindow::createRecoveryCopy()
{
    const VpResult<QString> result = m_recovery_manager->autosaveNow();
    if (!result)
    {
        VpDialogService::warning(this, tr("创建恢复副本"), toQtError(result));
        return;
    }
...
void VpCadMainWindow::showRecoveryManager()
{
    ...
    const QVector<VpRecoveryEntry> entries = m_recovery_manager->availableRecoveries();
    for (int index = 0; index < entries.size(); ++index)
    {
        const VpRecoveryEntry& entry = entries[index];
    ...
    connect(delete_button, &QPushButton::clicked, &dialog,
            [this, tree, entries, delete_button, restore_button]()
            {
                ...
                {
                    VpDialogService::warning(this, tr("删除恢复副本"),
                                             tr("部分恢复文件无法删除。"));
                    return;
                }
    ...
    connect(restore_button, &QPushButton::clicked, &dialog,
            [this, &dialog, tree, entries]()
            {
                ...
                const VpResult<void> result = m_recovery_manager->restoreRecovery(entries[index]);
                if (!result)
                {
                    VpDialogService::critical(this, tr("恢复失败"), toQtError(result));
                    return;
                }
