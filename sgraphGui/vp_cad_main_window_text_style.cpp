...
void VpCadMainWindow::showTextStyleManager()
{
    ...
    {
        const VpTextStyleRecord* style = m_document->textStyle(style_combo->currentText());
        if (!style)
        {
            return;
        }
    ...
    const auto refresh = [this, style_combo, load_style]()
    {
        ...
        style_combo->clear();
        for (const VpTextStyleRecord& style : m_document->textStyles())
        {
    ...
    connect(save_button, &QPushButton::clicked, &dialog,
             ...
             italic_check, refresh]()
            {
                ...
                {
                    VpDialogService::warning(this, tr("文字样式"), tr("样式参数无效。"));
                    return;
                }
                refresh();
            });
...
bool VpCadMainWindow::executeTextStyleCommand(const QString& command)
{
