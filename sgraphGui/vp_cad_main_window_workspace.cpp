...
QToolBar* VpCadMainWindow::createDrawingToolBar()
{
...
void VpCadMainWindow::createDockingWorkspace()
{
    const VpDesignToken& tokens = m_theme_manager.tokens();
    const auto icon_for = [&tokens](VpIconType icon_type)
    {
    ...
    m_workspace = new VpCadWorkspaceWidget();
    ...
    center_dock->setIcon(icon_for(VpIconType::Viewport));
    m_icon_docks.emplace_back(center_dock, VpIconType::Viewport);
    ...
    property_dock->setMinimumSizeHintMode(ads::CDockWidget::MinimumSizeHintFromContentMinimumSize);
    property_dock->setIcon(icon_for(VpIconType::Properties));
    m_icon_docks.emplace_back(property_dock, VpIconType::Properties);
                                     ...
                                     property_summary_label]()
    {
        ...
        for (const VpLayerRecord& layer_record : m_document->layers())
        {
    ...
    connect(m_workspace->viewport(), &VpCadViewport::selectionChanged, property_tree,
            [refresh_properties](const QVector<quint64>&)
            {
                refresh_properties();
            });
    connect(m_document.get(), &VpCadDocument::documentChanged, property_tree, refresh_properties);
    connect(m_document.get(), &VpCadDocument::layersChanged, property_tree, refresh_properties);
    connect(&m_theme_manager, &VpThemeManager::themeChanged, property_panel,
            [apply_property_style, refresh_properties](VpThemeMode)
            {
    ...
    connect(property_color_button, &QPushButton::clicked, this,
            [this]()
            {
                ...
                std::vector<VpEntityId> entity_ids;
                ...
                {
                    VpDialogService::warning(this, tr("修改实体颜色"),
    ...
    auto* external_reference_dock = createListDock(m_dock_manager, tr("外部参照"),
                                                   ...
                                                   icon_for(VpIconType::ExternalReference), this);
    m_icon_docks.emplace_back(external_reference_dock, VpIconType::ExternalReference);
    ...
    auto* tools_dock =
        createListDock(m_dock_manager, tr("工具选项板"), {tr("直线"), tr("圆"), tr("常用标注")},
                       icon_for(VpIconType::ToolPalette), this);
    m_icon_docks.emplace_back(tools_dock, VpIconType::ToolPalette);
    ...
    auto* design_center_dock = createListDock(m_dock_manager, tr("设计中心"),
                                              ...
                                              icon_for(VpIconType::DesignCenter), this);
    m_icon_docks.emplace_back(design_center_dock, VpIconType::DesignCenter);
    ...
    auto* sheet_set_dock =
        createListDock(m_dock_manager, tr("图纸集"), {tr("图纸"), tr("视图"), tr("模型视图")},
                       icon_for(VpIconType::SheetSet), this);
    m_icon_docks.emplace_back(sheet_set_dock, VpIconType::SheetSet);
    m_dock_manager->addDockWidget(ads::CenterDockWidgetArea, sheet_set_dock,
                                  layer_dock->dockAreaWidget());

    m_command_line = new VpCommandLineWidget();
    ...
    command_dock->setMinimumSizeHintMode(ads::CDockWidget::MinimumSizeHintFromContentMinimumSize);
    command_dock->setIcon(icon_for(VpIconType::CommandLine));
    m_icon_docks.emplace_back(command_dock, VpIconType::CommandLine);
    ...
    report_dock->setIcon(icon_for(VpIconType::CompatibilityReport));
    m_icon_docks.emplace_back(report_dock, VpIconType::CompatibilityReport);
    auto* report_view = new QTextBrowser(report_dock);
    report_view->setText(tr("当前原生 .vectorpath/.smartcam/.smartcad 格式无兼容性警告。\n"
    ...
    const auto add_dock_shortcut = [this](const QKeySequence& key_sequence,
                                          ads::CDockWidget* dock_widget, const QString& command_id,
                                          VpIconType icon_type)
    {
        QAction* shortcut = createAction(tr("显示%1").arg(dock_widget->windowTitle()), icon_type);
    ...
    add_dock_shortcut(QKeySequence(QStringLiteral("Ctrl+3")), tools_dock,
                      QStringLiteral("ui.tool_palette"), VpIconType::ToolPalette);
    add_dock_shortcut(QKeySequence(QStringLiteral("Ctrl+9")), command_dock,
                      QStringLiteral("ui.command_line"), VpIconType::CommandLine);
    QAction* command_search_shortcut = createAction(tr("命令搜索"), VpIconType::CommandLine);
    ...
    connect(command_search_shortcut, &QAction::triggered, m_command_line,
            &VpCommandLineWidget::focusInput);

    connect(m_command_line, &VpCommandLineWidget::commandSubmitted, this,
            &VpCadMainWindow::executeCommand);
    connect(m_command_line, &VpCommandLineWidget::commandInputChanged, this,
            &VpCadMainWindow::previewCommandInput);
    connect(m_workspace->viewport(), &VpCadViewport::commandMessage, m_command_line,
            &VpCommandLineWidget::appendMessage);
    connect(m_workspace->viewport(), &VpCadViewport::toolModeChanged, this,
            &VpCadMainWindow::updateToolButtonHighlight);
    updateToolButtonHighlight(VpToolMode::Select);
}

void VpCadMainWindow::configureDockSplitters()
{
