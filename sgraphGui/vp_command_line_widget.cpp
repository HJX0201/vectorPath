...
VpCommandLineWidget::VpCommandLineWidget(QWidget* parent) : QWidget(parent)
{
    ...
    connect(m_input, &QLineEdit::textChanged, this, &VpCommandLineWidget::commandInputChanged);
...
bool VpCommandLineWidget::eventFilter(QObject* watched, QEvent* event)
{
...
void VpCommandLineWidget::appendMessage(const QString& message)
{
    m_history->append(message.toHtmlEscaped());
}

void VpCommandLineWidget::focusInput()
{
    m_input->setFocus();
    m_input->selectAll();
}
