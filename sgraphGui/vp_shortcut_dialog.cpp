...
{
    return sequence.isEmpty() ? VpShortcutDialog::tr("未设置")
                              : sequence.toString(QKeySequence::NativeText);
}

} // namespace

VpShortcutDialog::VpShortcutDialog(VpShortcutManager& shortcut_manager, QWidget* parent)
    : QDialog(parent), m_shortcut_manager(shortcut_manager)
{
    ...
    auto* description = new QLabel(
        tr("选择功能后录入新的组合键。重复快捷键会被阻止，设置将在下次启动时保留。"), this);
    ...
    connect(m_search_edit, &QLineEdit::textChanged, this,
            [this](const QString& search_text)
            {
                ...
                for (int index = 0; index < m_tree->topLevelItemCount(); ++index)
                {
                    ...
                    const bool is_match = normalized.isEmpty() ||
    ...
    connect(apply_button, &QPushButton::clicked, this, &VpShortcutDialog::applySelectedShortcut);
    connect(clear_button, &QPushButton::clicked, this, &VpShortcutDialog::clearSelectedShortcut);
    connect(defaults_button, &QPushButton::clicked, this,
            &VpShortcutDialog::restoreDefaultShortcuts);
    connect(button_box, &QDialogButtonBox::rejected, this, &QDialog::reject);
    updateEditor(m_tree->currentItem());
}

void VpShortcutDialog::populateTree()
{
    m_tree->clear();
    for (const VpShortcutBinding& binding : m_shortcut_manager.bindings())
    {
        auto* item = new QTreeWidgetItem({binding.display_name, binding.command_id,
                                          sequenceText(binding.currentSequence()),
...
void VpShortcutDialog::refreshTreeValues()
{
    for (int index = 0; index < m_tree->topLevelItemCount(); ++index)
    {
        QTreeWidgetItem* item = m_tree->topLevelItem(index);
        const VpShortcutBinding* binding =
...
void VpShortcutDialog::updateEditor(QTreeWidgetItem* item)
{
    const VpShortcutBinding* binding =
...
void VpShortcutDialog::applySelectedShortcut()
{
    ...
    if (!m_shortcut_manager.setShortcut(command_id, m_sequence_edit->keySequence(),
                                        &conflicting_id))
    {
        const VpShortcutBinding* conflict = m_shortcut_manager.binding(conflicting_id);
        VpDialogService::warning(this, tr("快捷键冲突"),
...
void VpShortcutDialog::clearSelectedShortcut()
{
    QTreeWidgetItem* item = m_tree->currentItem();
    if (!item)
    {
...
void VpShortcutDialog::restoreDefaultShortcuts()
{
    if (VpDialogService::question(this, tr("恢复默认快捷键"), tr("确定恢复全部默认快捷键吗？")) !=
        QMessageBox::Yes)
    {
        return;
    }
