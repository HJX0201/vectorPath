...
QKeySequence VpShortcutBinding::currentSequence() const
{
...
void VpShortcutManager::registerAction(QAction* action, const QString& command_id,
                                       const QKeySequence& default_sequence)
{
    ...
    VpShortcutBinding& binding = m_bindings[index];
    ...
    const QKeySequence active_sequence = storedSequence(normalized_id, binding.default_sequence);
    for (const QPointer<QAction>& registered_action : binding.actions)
    {
        ...
        registered_action->setProperty(
            "smartDefaultShortcut", binding.default_sequence.toString(QKeySequence::PortableText));
        registered_action->setShortcutContext(Qt::WindowShortcut);
    }
    applySequence(binding, active_sequence);
}

QVector<VpShortcutBinding> VpShortcutManager::bindings() const
{
    QVector<VpShortcutBinding> result = m_bindings;
    std::sort(result.begin(), result.end(),
              [](const VpShortcutBinding& first, const VpShortcutBinding& second)
              {
...
const VpShortcutBinding* VpShortcutManager::binding(const QString& command_id) const
{
    const int index = bindingIndex(command_id.trimmed().toLower());
    return index < 0 ? nullptr : &m_bindings[index];
}

bool VpShortcutManager::setShortcut(const QString& command_id, const QKeySequence& shortcut,
                                    QString* conflicting_command_id)
{
    ...
    QSettings settings;
    settings.setValue(settingKey(normalized_id), shortcut.toString(QKeySequence::PortableText));
    settings.sync();
    applySequence(m_bindings[target_index], shortcut);
    return true;
}

bool VpShortcutManager::clearShortcut(const QString& command_id)
{
    return setShortcut(command_id, {});
}

void VpShortcutManager::restoreDefaults()
{
    QSettings settings;
    for (VpShortcutBinding& binding : m_bindings)
    {
...
int VpShortcutManager::bindingIndex(const QString& command_id) const
{
    for (int index = 0; index < m_bindings.size(); ++index)
    {
...
QKeySequence VpShortcutManager::storedSequence(const QString& command_id,
...
void VpShortcutManager::applySequence(VpShortcutBinding& binding, const QKeySequence& shortcut)
{
