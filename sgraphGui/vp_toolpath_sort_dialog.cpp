...
VpToolpathSortDialog::VpToolpathSortDialog(const VpToolpathSortOptions& initial_options,
                                           QWidget* parent)
    : QDialog(parent)
{
    ...
    auto* description = new QLabel(
        tr("设置当前选择集或当前空间内可加工实体的排序方式。实体将按图层依次处理。"), this);
    ...
    m_row_scan_check->setChecked(initial_options.mode == VpToolpathSortMode::RowScan);
    m_shortest_check->setChecked(initial_options.mode == VpToolpathSortMode::Shortest);
    m_left_to_right_check->setChecked(initial_options.horizontal ==
                                      VpToolpathHorizontalDirection::LeftToRight);
    m_right_to_left_check->setChecked(initial_options.horizontal ==
                                      VpToolpathHorizontalDirection::RightToLeft);
    m_top_to_bottom_check->setChecked(initial_options.vertical ==
                                      VpToolpathVerticalDirection::TopToBottom);
    m_bottom_to_top_check->setChecked(initial_options.vertical ==
...
VpToolpathSortOptions VpToolpathSortDialog::options() const
{
    VpToolpathSortOptions result;
    result.mode =
        m_shortest_check->isChecked() ? VpToolpathSortMode::Shortest : VpToolpathSortMode::RowScan;
    result.horizontal = m_right_to_left_check->isChecked()
                            ? VpToolpathHorizontalDirection::RightToLeft
                            : VpToolpathHorizontalDirection::LeftToRight;
    result.vertical = m_bottom_to_top_check->isChecked() ? VpToolpathVerticalDirection::BottomToTop
                                                         : VpToolpathVerticalDirection::TopToBottom;
    result.allow_reverse =
        result.mode == VpToolpathSortMode::Shortest && m_reverse_check->isChecked();
    return result;
}

void VpToolpathSortDialog::updateControlStates()
{
