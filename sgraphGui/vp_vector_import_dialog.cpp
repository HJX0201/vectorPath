...
QRectF entityBounds(const VpColoredEntityGeometry& entity)
{
    if (entity.type == VpEntityType::Line)
    {
        const VpLineEntity& line = std::get<VpLineEntity>(entity.geometry);
        return QRectF(toQtPoint(line.start_point), toQtPoint(line.end_point)).normalized();
    }
    if (entity.type == VpEntityType::Polyline)
    {
        const VpPolylineEntity& polyline = std::get<VpPolylineEntity>(entity.geometry);
        if (polyline.vertices.empty())
        {
            return {};
        }
        QRectF bounds(toQtPoint(polyline.vertices.front()), QSizeF());
        for (const VpPoint2d& point : polyline.vertices)
        {
            bounds |= QRectF(toQtPoint(point), QSizeF());
        }
        return bounds;
    }
    if (entity.type == VpEntityType::Hatch)
    {
        ...
        QRectF bounds(toQtPoint(hatch.boundary.front()), QSizeF());
...
QRectF geometryBounds(const VpVectorImportGeometry& geometry)
{
    QRectF result;
    bool has_bounds = false;
    for (const VpColoredEntityGeometry& entity : geometry.entities)
    {
...
void drawEntity(QPainter& painter, const VpColoredEntityGeometry& entity,
                const QRectF& world_bounds, const QRectF& target)
{
    ...
    const auto map_point = [&](const VpPoint2d& point)
    {
        return QPointF(target.center().x() + (point.x - world_bounds.center().x()) * scale,
                       target.center().y() - (point.y - world_bounds.center().y()) * scale);
    };
    QColor color = entity.color;
    color.setAlpha(std::max(72, color.alpha()));
    painter.setPen(QPen(color, entity.type == VpEntityType::Line ? 1.0 : 1.4));
    if (entity.type == VpEntityType::Line)
    {
        const VpLineEntity& line = std::get<VpLineEntity>(entity.geometry);
    ...
    else if (entity.type == VpEntityType::Hatch)
    {
        ...
        const auto add_loop = [&](const std::vector<VpPoint2d>& loop)
        {
            if (loop.empty())
            {
                return;
...
VpVectorImportDialog::VpVectorImportDialog(QByteArray svg_data, QWidget* parent)
...
VpVectorImportDialog::VpVectorImportDialog(QImage bitmap, QWidget* parent)
...
VpVectorImportDialog::VpVectorImportDialog(VpSvgVectorData vector_data, QWidget* parent)
...
const VpVectorImportGeometry& VpVectorImportDialog::importGeometry() const
{
    return *m_geometry;
}

VpVectorImportSettings VpVectorImportDialog::vectorSettings() const
{
    return settings();
}

QByteArray VpVectorImportDialog::generatedSvgData() const
{
    return m_svg_data;
}

void VpVectorImportDialog::createInterface()
{
    resize(820, 620);
    auto* root_layout = new QVBoxLayout(this);
    auto* settings_group = new QGroupBox(m_is_fill_only ? tr("SVG 填充") : tr("SVG 导入"), this);
...
void VpVectorImportDialog::connectControls()
{
    ...
    if (m_ignore_background)
    {
        ...
        connect(m_background_color, &QPushButton::clicked, this,
                &VpVectorImportDialog::chooseBackgroundColor);
        connect(m_top_left_color, &QPushButton::clicked, this,
                &VpVectorImportDialog::useTopLeftBackground);
    }
}

void VpVectorImportDialog::rebuildVectorData()
{
    if (!m_bitmap.isNull())
    {
        VpBitmapVectorSettings bitmap_settings;
        bitmap_settings.ignore_background = m_ignore_background->isChecked();
        bitmap_settings.background_color = m_selected_background;
        const VpResult<QByteArray> vector_result = bitmapToSvgData(m_bitmap, bitmap_settings);
        if (!vector_result)
        {
            m_geometry.reset();
            m_summary->setText(toQtError(vector_result));
            m_buttons->button(QDialogButtonBox::Ok)->setEnabled(false);
            return;
        }
        m_svg_data = vector_result.value();
    }
    const VpResult<VpSvgVectorData> parse_result = parseSvgVectorData(m_svg_data);
    if (!parse_result)
    {
        m_geometry.reset();
        m_summary->setText(toQtError(parse_result));
        m_buttons->button(QDialogButtonBox::Ok)->setEnabled(false);
        return;
    }
...
void VpVectorImportDialog::updatePreview()
{
    if (!m_geometry)
    {
        return;
    ...
    for (const VpColoredEntityGeometry& entity : m_geometry->entities)
    {
        drawEntity(painter, entity, bounds, target);
    }
    m_preview->setPixmap(
        QPixmap::fromImage(preview_image)
            .scaled(m_preview->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
    QString summary = tr("尺寸：%1 × %2　区域：%3　预计原生实体：%4")
...
void VpVectorImportDialog::chooseBackgroundColor()
{
    const QColor selected = QColorDialog::getColor(
        m_selected_background, this, tr("选择要忽略的背景色"), QColorDialog::ShowAlphaChannel);
    if (selected.isValid())
    {
...
void VpVectorImportDialog::useTopLeftBackground()
{
    if (!m_bitmap.isNull())
    {
        m_selected_background = QColor::fromRgba(m_bitmap.pixel(0, 0));
...
VpVectorImportSettings VpVectorImportDialog::settings() const
{
    VpVectorImportSettings result;
    if (m_fill_mode->currentIndex() == 0)
    {
        result.fill_mode = VpImportFillMode::SingleLine;
    }
    else if (m_fill_mode->currentIndex() == 1)
    {
        result.fill_mode = VpImportFillMode::PolygonOffset;
    }
    else if (m_fill_mode->currentIndex() == 2)
    {
        result.fill_mode = VpImportFillMode::None;
    }
