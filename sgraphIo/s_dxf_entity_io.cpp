...
SEntityRecord commonRecord(const std::vector<SDxfPair>& pairs)
{
...
std::vector<SPoint2d> readRepeatedPoints(const std::vector<SDxfPair>& pairs, int x_code,
                                         int y_code)
{
...
SResult<SEntityRecord> invalidEntity(const QString& entity_name)
{
    return SResult<SEntityRecord>::failure(
        QObject::tr("字段不完整或几何无效的 %1 实体。").arg(entity_name));
}

void countExport(SFileCompatibilityReport& report, bool count_entity)
{
    if (count_entity)
    {
...
{
    SEntityRecord entity = commonRecord(entity_pairs);
    if (entity_name == QLatin1String("LINE"))
    {
        ...
        entity.geometry = SLineEntity{{*x1, *y1}, {*x2, *y2}};
    }
    else if (entity_name == QLatin1String("CIRCLE"))
    {
        ...
        entity.geometry = SCircleEntity{{*center_x, *center_y}, *radius};
    }
    else if (entity_name == QLatin1String("ARC"))
    {
        ...
        entity.geometry = SArcEntity{{*center_x, *center_y}, *radius, *start_angle, *end_angle};
    }
    else if (entity_name == QLatin1String("ELLIPSE"))
    {
        ...
        entity.geometry = SEllipseEntity{{*center_x, *center_y}, {*major_x, *major_y},
    ...
    else if (entity_name == QLatin1String("DIMENSION"))
    {
        ...
        const int raw_dxf_type =
            static_cast<int>(findDxfDouble(entity_pairs, 70).value_or(0.0));
        int dimension_type = smartcad_type ? static_cast<int>(*smartcad_type) : (raw_dxf_type & 7);
        if (!smartcad_type && dimension_type == 3)
        {
            dimension_type = static_cast<int>(SDimensionType::Diameter);
        }
        else if (!smartcad_type && dimension_type == 4)
        {
            dimension_type = static_cast<int>(SDimensionType::Radius);
        }
        else if (!smartcad_type && dimension_type == 6)
        {
            dimension_type = static_cast<int>(SDimensionType::Ordinate);
        }
        if (dimension_type >= 0 && dimension_type <= static_cast<int>(SDimensionType::Ordinate))
        {
            dimension.dimension_type = static_cast<SDimensionType>(dimension_type);
        }
        dimension.style_name =
            findDxfString(entity_pairs, 3).value_or(QStringLiteral("Standard"));
    ...
    {
        SResult<SHatchEntity> hatch = readDxfHatch(entity_pairs);
        if (!hatch)
        {
            return SResult<SEntityRecord>::failure(hatch.errorMessage());
        }
