...
{
    return (static_cast<quint64>(static_cast<quint32>(point.x())) << 32U) |
           static_cast<quint32>(point.y());
}

struct VpNormalizedSegment
{
    int block_id = -1;
    QRgb color = 0;
    int direction = 0;
...
std::vector<QPoint> simplifyLoop(const std::vector<QPoint>& source)
{
    ...
    {
        const QPoint& previous = source[(index + source.size() - 1) % source.size()];
        ...
        if (first_delta.x() * second_delta.y() - first_delta.y() * second_delta.x() != 0)
        {
...
void canonicalizeLoop(std::vector<QPoint>& points)
{
    ...
    {
        if (std::make_tuple(iterator->y(), iterator->x()) < std::make_tuple(best->y(), best->x()))
        {
            best = iterator;
        }
    }
...
{
    std::vector<VpNormalizedSegment> normalized;
    normalized.reserve(source.size());
    for (const VpBoundarySegment& segment : source)
    {
        if (segment.start == segment.end)
        {
            return VpResult<std::vector<VpBoundarySegment>>::failure(
                toCoreText(QStringLiteral("位图边界包含零长度线段。")));
        }
        normalized.push_back(normalizeSegment(segment));
    }
    std::sort(normalized.begin(), normalized.end(),
              [](const VpNormalizedSegment& first, const VpNormalizedSegment& second)
              {
                  return std::tie(first.block_id, first.color, first.direction, first.fixed,
                                  first.low, first.high) < std::tie(second.block_id, second.color,
                                                                    second.direction, second.fixed,
                                                                    second.low, second.high);
              });

    std::vector<VpNormalizedSegment> merged;
    for (const VpNormalizedSegment& segment : normalized)
    {
        if (!merged.empty())
        {
            VpNormalizedSegment& previous = merged.back();
            const bool is_same_line =
                previous.block_id == segment.block_id && previous.color == segment.color &&
                previous.direction == segment.direction && previous.fixed == segment.fixed;
            if (is_same_line && segment.low < previous.high)
            {
                return VpResult<std::vector<VpBoundarySegment>>::failure(
                    toCoreText(QStringLiteral("位图边界包含重复或重叠线段。")));
            }
            if (is_same_line && segment.low == previous.high)
            {
    ...
    std::vector<VpBoundarySegment> result;
    result.reserve(merged.size());
    for (const VpNormalizedSegment& segment : merged)
    {
        result.push_back(restoreSegment(segment));
    }
    return VpResult<std::vector<VpBoundarySegment>>::success(std::move(result));
}
...
VpResult<std::vector<VpBitmapContour>> stitchSegments(std::vector<VpBoundarySegment>& segments)
{
    ...
    while (range_begin < segments.size())
    {
        ...
        while (range_end < segments.size() &&
               segments[range_end].block_id == segments[range_begin].block_id)
        {
            ++range_end;
        }
        ...
        {
            outgoing[pointKey(segments[index].start)].append(static_cast<int>(index));
        }
        for (std::size_t start_index = range_begin; start_index < range_end; ++start_index)
        {
            ...
            {
                const std::size_t current_index = static_cast<std::size_t>(edge_index);
                if (is_used[current_index])
                {
                    return VpResult<std::vector<VpBitmapContour>>::failure(
                        toCoreText(QStringLiteral("位图轮廓在闭合前重复使用了线段。")));
                }
                is_used[current_index] = true;
                const VpBoundarySegment& edge = segments[current_index];
                ...
                for (int candidate_index : outgoing.value(pointKey(vertex)))
                {
                    ...
                    const int priority = turnPriority(
                        previous_direction,
                        segmentDirection(segments[static_cast<std::size_t>(candidate_index)]));
                ...
                {
                    return VpResult<std::vector<VpBitmapContour>>::failure(
                        toCoreText(QStringLiteral("位图轮廓存在开放端点或无法消解的分支。")));
                }
            ...
            if (loop.empty() || segments[static_cast<std::size_t>(edge_index)].end != loop_start)
            {
                return VpResult<std::vector<VpBitmapContour>>::failure(
                    toCoreText(QStringLiteral("位图轮廓未能闭合。")));
            }
            loop = simplifyLoop(loop);
            if (loop.size() < 3)
            {
                return VpResult<std::vector<VpBitmapContour>>::failure(
                    toCoreText(QStringLiteral("位图轮廓退化为少于三个顶点。")));
            }
            canonicalizeLoop(loop);
            contours.push_back(
                {segments[start_index].color, segments[start_index].block_id, std::move(loop)});
        }
        range_begin = range_end;
    }
    std::sort(contours.begin(), contours.end(),
              [](const VpBitmapContour& first, const VpBitmapContour& second)
              {
                  ...
                  return std::make_tuple(first.points.front().y(), first.points.front().x()) <
                         std::make_tuple(second.points.front().y(), second.points.front().x());
              });
    return VpResult<std::vector<VpBitmapContour>>::success(std::move(contours));
}
