...
QColor parseColor(const QString& text, QSet<QString>& warning_keys, QStringList& warnings)
{
    ...
    static const QRegularExpression rgb_pattern(QStringLiteral(R"(^rgba?\s*\(([^)]+)\)$)"),
...
QHash<QString, QString> inlineStyle(const QString& text)
{
    ...
    for (const QString& declaration : text.split(QLatin1Char(';'), QString::SkipEmptyParts))
    {
...
VpSvgStyle elementStyle(const QXmlStreamAttributes& attributes, const VpSvgStyle& inherited,
                        QSet<QString>& warning_keys, QStringList& warnings)
{
    VpSvgStyle result = inherited;
    ...
    if (properties.value(QStringLiteral("fill-rule"))
            .compare(QStringLiteral("evenodd"), Qt::CaseInsensitive) == 0)
    {
        result.fill_rule = VpVectorFillRule::EvenOdd;
    }
    if (properties.value(QStringLiteral("display"))
                .compare(QStringLiteral("none"), Qt::CaseInsensitive) == 0 ||
        properties.value(QStringLiteral("visibility"))
                .compare(QStringLiteral("hidden"), Qt::CaseInsensitive) == 0)
    {
...
QTransform svgTransform(const QString& text)
{
    ...
    while (iterator.hasNext())
    {
        ...
        {
            transform =
                QTransform(values[0], values[1], values[2], values[3], values[4], values[5]);
        }
...
std::vector<VpSvgSubpath> elementPaths(const QString& name, const QXmlStreamAttributes& attributes,
                                       QString& error_message)
{
    std::vector<VpSvgSubpath> result;
    QPainterPath path;
    if (name == QLatin1String("path"))
    {
        ...
        if (!parsed)
        {
            error_message = toQtError(parsed);
            return {};
        }
        return parsed.value();
    }
    ...
    else if (name == QLatin1String("rect"))
    {
        ...
        const double radius_y = number(attributes.value(QStringLiteral("ry")).toString(), radius_x);
    ...
    {
        result.push_back(
            {path, name != QLatin1String("line") && name != QLatin1String("polyline")});
    }
    return result;
}

std::vector<VpPoint2d> flattenedPoints(const QPainterPath& path, const QTransform& transform)
{
    ...
    {
        if (result.empty() || QLineF(toQtPoint(result.back()), point).length() > 1.0e-8)
        {
            result.push_back(toCorePoint(point));
        }
    }
    return result;
}

QColor outlineColor(const VpSvgStyle& style)
{
    ...
    applyOpacity(result, style.opacity *
                             (style.stroke.isValid() ? style.stroke_opacity : style.fill_opacity));
    return result;
}

QColor regionColor(const VpSvgStyle& style)
{
    ...
    applyOpacity(result, style.opacity *
                             (style.fill.isValid() ? style.fill_opacity : style.stroke_opacity));
    return result;
}

void normalizeCoordinates(VpSvgVectorData& data, VpSvgViewport viewport)
{
    double maximum_x = 0.0;
    double maximum_y = 0.0;
    bool has_point = false;
    const auto inspect = [&](const std::vector<VpPoint2d>& points)
    {
    ...
    for (const VpVectorOutline& outline : data.outlines)
    {
    ...
    const auto normalize = [&](std::vector<VpPoint2d>& points)
    {
    ...
    for (VpVectorOutline& outline : data.outlines)
    {
        normalize(outline.points);
    }
    for (VpVectorRegion& region : data.regions)
    {
        for (std::vector<VpPoint2d>& contour : region.contours)
        {
            normalize(contour);
        }
    }
...
VpResult<VpSvgVectorData> parseSvgVectorData(const QByteArray& svg_data)
{
    ...
    {
        return VpResult<VpSvgVectorData>::failure(toCoreText(QStringLiteral("SVG 数据为空。")));
    }
    QXmlStreamReader reader(svg_data);
    VpSvgVectorData result;
    VpSvgViewport viewport;
    std::vector<VpSvgContext> contexts{{}};
    QSet<QString> warning_keys;
    int root_depth = 0;
    const QSet<QString> geometry_elements{QStringLiteral("path"),    QStringLiteral("line"),
                                          QStringLiteral("rect"),    QStringLiteral("circle"),
                                          QStringLiteral("ellipse"), QStringLiteral("polyline"),
                                          QStringLiteral("polygon")};
    const QSet<QString> unsupported_elements{QStringLiteral("text"), QStringLiteral("image"),
                                             QStringLiteral("use"),  QStringLiteral("filter"),
                                             QStringLiteral("mask"), QStringLiteral("clipPath")};
    ...
    while (!reader.atEnd())
    {
        ...
        if (reader.isStartElement())
        {
            ...
            VpSvgContext context = contexts.back();
            context.style = elementStyle(attributes, context.style, warning_keys, result.warnings);
            context.transform =
            ...
            if (name == QLatin1String("svg") && root_depth == 1)
            {
                ...
                {
                    viewport.width = number(attributes.value(QStringLiteral("width")).toString());
                    viewport.height = number(attributes.value(QStringLiteral("height")).toString());
                }
            }
            ...
            QString path_error;
            const std::vector<VpSvgSubpath> subpaths = elementPaths(name, attributes, path_error);
            if (!path_error.isEmpty())
            {
                return VpResult<VpSvgVectorData>::failure(toCoreText(path_error));
            }
            VpVectorRegion region;
            region.color = regionColor(context.style);
            region.fill_rule = context.style.fill_rule;
            for (const VpSvgSubpath& subpath : subpaths)
            {
                std::vector<VpPoint2d> points = flattenedPoints(subpath.path, context.transform);
                if (points.size() < 2)
                {
                    continue;
                }
                ...
                result.outlines.push_back({points, subpath.is_closed, outlineColor(context.style)});
    ...
    if (reader.hasError())
    {
        return VpResult<VpSvgVectorData>::failure(
            toCoreText(QStringLiteral("SVG XML 解析失败：%1").arg(reader.errorString())));
    }
    if (result.outlines.empty())
    {
        return VpResult<VpSvgVectorData>::failure(
            toCoreText(QStringLiteral("SVG 中没有可导入的矢量几何。")));
    }
    normalizeCoordinates(result, viewport);
    return VpResult<VpSvgVectorData>::success(std::move(result));
}
