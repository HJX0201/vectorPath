...
class VpPathReader final
{
  public:
    explicit VpPathReader(const QString& text) : m_text(text)
    {
    }
    ...
    bool readNumber(double& result)
    {
        ...
        if (m_text.at(m_index) == QLatin1Char('+') || m_text.at(m_index) == QLatin1Char('-'))
        {
            ++m_index;
        }
        ...
        if (m_index < m_text.size() &&
            (m_text.at(m_index) == QLatin1Char('e') || m_text.at(m_index) == QLatin1Char('E')))
        {
            const int exponent_start = m_index++;
            if (m_index < m_text.size() &&
                (m_text.at(m_index) == QLatin1Char('+') || m_text.at(m_index) == QLatin1Char('-')))
            {
                ++m_index;
            }
...
void appendSvgArc(QPainterPath& path, const QPointF& start, const QPointF& end, double radius_x,
                  double radius_y, double rotation_degrees, bool large_arc, bool sweep)
{
    ...
    double radii_scale =
        x_prime * x_prime / (radius_x * radius_x) + y_prime * y_prime / (radius_y * radius_y);
    if (radii_scale > 1.0)
    {
    ...
    const double numerator = std::max(0.0, radius_x * radius_x * radius_y * radius_y -
                                               radius_x * radius_x * y_prime * y_prime -
                                               radius_y * radius_y * x_prime * x_prime);
    const double denominator =
        radius_x * radius_x * y_prime * y_prime + radius_y * radius_y * x_prime * x_prime;
    ...
    const double start_angle = vectorAngle(1.0, 0.0, (x_prime - center_x_prime) / radius_x,
                                           (y_prime - center_y_prime) / radius_y);
    double sweep_angle =
        vectorAngle((x_prime - center_x_prime) / radius_x, (y_prime - center_y_prime) / radius_y,
                    (-x_prime - center_x_prime) / radius_x, (-y_prime - center_y_prime) / radius_y);
    ...
    for (int segment = 0; segment < segment_count; ++segment)
    {
        ...
        {
            return QPointF(center_x + cosine * radius_x * unit_x - sine * radius_y * unit_y,
                           center_y + sine * radius_x * unit_x + cosine * radius_y * unit_y);
        };
        ...
        const QPointF first_control = map_point(std::cos(angle) - factor * std::sin(angle),
...
bool readPair(VpPathReader& reader, double& first, double& second)
{
...
{
    VpPathReader reader(path_data);
    std::vector<VpSvgSubpath> result;
    VpSvgSubpath* current_subpath = nullptr;
    QPointF current;
    ...
    const auto ensure_subpath = [&]() -> VpSvgSubpath&
    {
        if (!current_subpath)
        {
            result.push_back({});
    ...
    while (!reader.atEnd())
    {
        ...
        {
            return VpResult<std::vector<VpSvgSubpath>>::failure(
                toCoreText(QStringLiteral("SVG path 缺少命令。")));
        }
        ...
        {
            return VpResult<std::vector<VpSvgSubpath>>::failure(
                toCoreText(QStringLiteral("SVG path 包含不支持的命令：%1").arg(command)));
        }
        for (int index = 0; index < required; ++index)
        {
            ...
            {
                return VpResult<std::vector<VpSvgSubpath>>::failure(
                    toCoreText(QStringLiteral("SVG path 命令 %1 的参数不完整。").arg(command)));
            }
        }
        ...
        else
        {
            ...
            else if (normalized == QLatin1Char('S'))
            {
                ...
                const QPointF first = previous == QLatin1Char('C') || previous == QLatin1Char('S')
            ...
            else if (normalized == QLatin1Char('T'))
            {
                ...
                const QChar previous = previous_command.toUpper();
                const QPointF control = previous == QLatin1Char('Q') || previous == QLatin1Char('T')
    ...
    result.erase(std::remove_if(result.begin(), result.end(),
                                [](const VpSvgSubpath& subpath)
                                {
                                    return subpath.path.elementCount() < 2;
                                }),
                 result.end());
    return VpResult<std::vector<VpSvgSubpath>>::success(std::move(result));
}
