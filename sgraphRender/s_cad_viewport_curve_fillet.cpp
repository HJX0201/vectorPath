...
struct SFilletCandidate
{
    SPoint2d center;
    SPoint2d line_tangent;
    SPoint2d curve_tangent;
    double score = std::numeric_limits<double>::max();
};

void appendCandidateCenters(const SPoint2d& line_origin, const SPoint2d& line_direction,
                            const SPoint2d& curve_center, double center_distance,
...
std::vector<SPoint2d> circleIntersections(const SPoint2d& first_center, double first_radius,
...
bool curveGeometry(const SEntityRecord& source, SPoint2d& center, double& radius)
{
...
void trimArcResult(const SEntityRecord& source, const SPoint2d& pick_point,
                   const SPoint2d& tangent_point, SEntityRecord& result)
{
    result = source;
    if (source.type != SEntityType::Arc)
    {
    ...
    const auto& source_arc = std::get<SArcEntity>(source.geometry);
    auto& result_arc = std::get<SArcEntity>(result.geometry);
    const SPoint2d start_point = arcPoint(source_arc, source_arc.start_angle);
    const SPoint2d end_point = arcPoint(source_arc, source_arc.end_angle);
...
bool curveCurveFillet(const SEntityRecord& first_source, const SEntityRecord& second_source,
...
{
    SPoint2d first_center;
    SPoint2d second_center;
    ...
    const SFilletCandidate& best =
        *std::min_element(candidates.begin(), candidates.end(),
                          [](const SFilletCandidate& first, const SFilletCandidate& second)
                          {
    ...
    fillet_result.type = SEntityType::Arc;
    fillet_result.geometry = SArcEntity{best.center, radius, start_angle, end_angle};
    return true;
}

bool curveFillet(const SEntityRecord& line_source, const SEntityRecord& curve_source,
                 const SPoint2d& line_pick, const SPoint2d& curve_pick, double radius,
                 SEntityRecord& line_result, SEntityRecord& curve_result,
                 SEntityRecord& fillet_result)
{
    const auto& line = std::get<SLineEntity>(line_source.geometry);
    ...
    const SPoint2d direction{line_x / line_length, line_y / line_length};
    const SPoint2d normal{-direction.y, direction.x};
    const SPoint2d curve_center = curve_source.type == SEntityType::Circle
                                      ? std::get<SCircleEntity>(curve_source.geometry).center
                                      : std::get<SArcEntity>(curve_source.geometry).center;
    const double curve_radius = curve_source.type == SEntityType::Circle
                                    ? std::get<SCircleEntity>(curve_source.geometry).radius
                                    : std::get<SArcEntity>(curve_source.geometry).radius;
    std::vector<SFilletCandidate> candidates;
    for (double side : {-1.0, 1.0})
    {
        const SPoint2d offset_origin{line.start_point.x + (side * radius * normal.x),
        ...
        for (const SPoint2d& center : centers)
        {
            ...
            const SPoint2d line_tangent{line.start_point.x + (projection * direction.x),
                                        line.start_point.y + (projection * direction.y)};
            const SPoint2d curve_tangent{
            ...
            if (curve_source.type == SEntityType::Arc &&
                !angleOnArc(entityAngleDegrees(curve_center, curve_tangent),
                            std::get<SArcEntity>(curve_source.geometry)))
            {
                continue;
            }
            candidates.push_back(
    ...
    const SPoint2d retained_line_point =
    ...
    line_result.geometry = SLineEntity{retained_line_point, best.line_tangent};
    curve_result = curve_source;
    if (curve_source.type == SEntityType::Arc)
    {
        const auto& source_arc = std::get<SArcEntity>(curve_source.geometry);
        auto& result_arc = std::get<SArcEntity>(curve_result.geometry);
        const SPoint2d start_point = arcPoint(source_arc, source_arc.start_angle);
        const SPoint2d end_point = arcPoint(source_arc, source_arc.end_angle);
                      ...
                      SEntityRecord& arc_result)
{
    ...
    const bool first_is_curve =
        first_source.type == SEntityType::Circle || first_source.type == SEntityType::Arc;
    const bool second_is_curve =
        second_source.type == SEntityType::Circle || second_source.type == SEntityType::Arc;
    if (first_source.type == SEntityType::Line && second_is_curve)
    {
        return curveFillet(first_source, second_source, first_pick, second_pick, radius,
                           first_result, second_result, arc_result);
    }
    if (first_is_curve && second_source.type == SEntityType::Line)
    {
