...
std::vector<SPoint2d> lineCircleIntersections(const SLineEntity& line, const SPoint2d& center,
                                              double radius)
{
                             ...
                             (-coefficient_b + root) / (2.0 * coefficient_a)})
    {
        ...
        {
            const SPoint2d point{line.start_point.x + (parameter * delta_x),
...
bool pointOnEntity(const SPoint2d& point, const SEntityRecord& entity)
{
...
std::vector<SPoint2d> curveIntersections(const SEntityRecord& first, const SEntityRecord& second)
{
    std::vector<SPoint2d> points;
    if (first.type == SEntityType::Line &&
        (second.type == SEntityType::Circle || second.type == SEntityType::Arc))
    {
        const SPoint2d center = second.type == SEntityType::Circle
                                    ? std::get<SCircleEntity>(second.geometry).center
                                    : std::get<SArcEntity>(second.geometry).center;
        const double radius = second.type == SEntityType::Circle
                                  ? std::get<SCircleEntity>(second.geometry).radius
                                  : std::get<SArcEntity>(second.geometry).radius;
        points = lineCircleIntersections(std::get<SLineEntity>(first.geometry), center, radius);
    }
    else if (second.type == SEntityType::Line &&
             (first.type == SEntityType::Circle || first.type == SEntityType::Arc))
    {
        points = curveIntersections(second, first);
    }
    else if (first.type != SEntityType::Line && second.type != SEntityType::Line)
    {
        const SPoint2d first_center = first.type == SEntityType::Circle
                                          ? std::get<SCircleEntity>(first.geometry).center
                                          : std::get<SArcEntity>(first.geometry).center;
        const double first_radius = first.type == SEntityType::Circle
                                        ? std::get<SCircleEntity>(first.geometry).radius
                                        : std::get<SArcEntity>(first.geometry).radius;
        const SPoint2d second_center = second.type == SEntityType::Circle
                                           ? std::get<SCircleEntity>(second.geometry).center
                                           : std::get<SArcEntity>(second.geometry).center;
        const double second_radius = second.type == SEntityType::Circle
                                         ? std::get<SCircleEntity>(second.geometry).radius
                                         : std::get<SArcEntity>(second.geometry).radius;
        points =
...
std::vector<SEntityRecord> trimLine(const SEntityRecord& target,
                                    const std::vector<SPoint2d>& intersections,
                                    const SPoint2d& pick_point)
{
    const auto& line = std::get<SLineEntity>(target.geometry);
    const double delta_x = line.end_point.x - line.start_point.x;
    const double delta_y = line.end_point.y - line.start_point.y;
    const double length_squared = (delta_x * delta_x) + (delta_y * delta_y);
    std::vector<double> boundaries{0.0, 1.0};
    for (const SPoint2d& point : intersections)
    {
    ...
    for (std::size_t index = 0; index + 1 < boundaries.size(); ++index)
    {
        ...
        SEntityRecord result = target;
        result.geometry = SLineEntity{point_at(boundaries[index]), point_at(boundaries[index + 1])};
        results.push_back(std::move(result));
    }
    return results;
}

double pointToSegmentDistance(const SPoint2d& point, const SPoint2d& start_point,
...
double polylineSegmentParameter(const SPoint2d& start_point, const SPoint2d& end_point,
                                double bulge, const SPoint2d& point)
{
...
SPoint2d polylineSegmentPoint(const SPoint2d& start_point, const SPoint2d& end_point, double bulge,
                              double parameter)
{
    if (std::abs(bulge) <= 1.0e-12)
    {
...
double pointToPolylineSegmentDistance(const SPoint2d& point, const SPoint2d& start_point,
                                      const SPoint2d& end_point, double bulge)
{
    if (std::abs(bulge) <= 1.0e-12)
    {
...
void appendPolylineEdge(std::vector<SPoint2d>& vertices, std::vector<double>& bulges,
                        const SPoint2d& start_point, const SPoint2d& end_point, double bulge)
{
...
void appendOpenPolylinePart(const SEntityRecord& source, std::vector<SPoint2d> vertices,
                            std::vector<double> bulges, std::vector<SEntityRecord>& results)
{
    ...
    result.geometry = SPolylineEntity{std::move(vertices), false, std::move(bulges)};
    results.push_back(std::move(result));
}

std::vector<SEntityRecord> trimOpenPolyline(const SEntityRecord& cutting_entity,
                                            ...
                                            const SPoint2d& pick_point)
{
    ...
    SEntityRecord segment = target_entity;
    const std::size_t segment_end = (picked_segment + 1) % polyline.vertices.size();
    const SPoint2d segment_start_point = polyline.vertices[picked_segment];
    const SPoint2d segment_end_point = polyline.vertices[segment_end];
    const double source_bulge =
        picked_segment < polyline.bulges.size() ? polyline.bulges[picked_segment] : 0.0;
    SArcEntity segment_arc;
    if (bulgeArc(segment_start_point, segment_end_point, source_bulge, segment_arc))
    {
        segment.type = SEntityType::Arc;
        segment.geometry = segment_arc;
    }
    else
    {
        segment.type = SEntityType::Line;
        segment.geometry = SLineEntity{segment_start_point, segment_end_point};
    }
    std::vector<SEntityRecord> kept_segments =
        trimmedEntityParts(cutting_entity, segment, pick_point);
    if (kept_segments.empty())
    {
        return {};
    }
    const auto parameter = [&](const SPoint2d& point)
    {
        return std::clamp(
    ...
    for (const SEntityRecord& kept_segment : kept_segments)
    {
        SPoint2d kept_start;
        SPoint2d kept_end;
        if (kept_segment.type == SEntityType::Line)
        {
            const auto& line = std::get<SLineEntity>(kept_segment.geometry);
            kept_start = line.start_point;
            kept_end = line.end_point;
        }
        else
        {
            const auto& arc = std::get<SArcEntity>(kept_segment.geometry);
            const auto point_at_angle = [&](double angle)
            {
                const double radians = angle * kPi / 180.0;
                return SPoint2d{arc.center.x + (arc.radius * std::cos(radians)),
    ...
    const auto add_source_edge =
        [&](std::size_t index, std::vector<SPoint2d>& vertices, std::vector<double>& bulges)
    {
        const std::size_t end_index = (index + 1) % polyline.vertices.size();
        const double bulge = index < polyline.bulges.size() ? polyline.bulges[index] : 0.0;
    ...
    const auto add_trimmed_edge = [&](double start_parameter, double end_parameter,
                                      std::vector<SPoint2d>& vertices, std::vector<double>& bulges)
    {
    ...
    {
        std::vector<SPoint2d> remaining_vertices;
    ...
    std::vector<SPoint2d> left_vertices;
    ...
    appendOpenPolylinePart(target_entity, std::move(left_vertices), std::move(left_bulges),
                           results);
    std::vector<SPoint2d> right_vertices;
                                              ...
                                              const SPoint2d& pick_point)
{
    ...
    if (cutting_entity.type == SEntityType::Line && target_entity.type == SEntityType::Line)
    {
        ...
        return trimmedLineEntity(cutting_entity, target_entity, pick_point, result)
                   ? std::vector<SEntityRecord>{result}
                   : std::vector<SEntityRecord>{};
    }
    const std::vector<SPoint2d> intersections = curveIntersections(cutting_entity, target_entity);
    ...
    if (target_entity.type == SEntityType::Circle)
    {
        if (intersections.size() < 2)
        {
            return {};
        }
        const auto& circle = std::get<SCircleEntity>(target_entity.geometry);
        ...
        SEntityRecord result = target_entity;
        result.type = SEntityType::Arc;
        result.geometry = pick_on_first_arc
                              ? SArcEntity{circle.center, circle.radius, second_angle, first_angle}
                              : SArcEntity{circle.center, circle.radius, first_angle, second_angle};
        return {result};
    }
    const SPoint2d center = target_entity.type == SEntityType::Circle
                                ? std::get<SCircleEntity>(target_entity.geometry).center
                                : std::get<SArcEntity>(target_entity.geometry).center;
    const double radius = target_entity.type == SEntityType::Circle
                              ? std::get<SCircleEntity>(target_entity.geometry).radius
                              : std::get<SArcEntity>(target_entity.geometry).radius;
    double start_angle = target_entity.type == SEntityType::Circle
                             ? 0.0
                             : std::get<SArcEntity>(target_entity.geometry).start_angle;
    const double total_span =
        target_entity.type == SEntityType::Circle
            ? 360.0
            : counterClockwiseSpan(start_angle,
                                   std::get<SArcEntity>(target_entity.geometry).end_angle);
    ...
    for (std::size_t index = 0; index + 1 < boundaries.size(); ++index)
    {
        ...
        result.type = SEntityType::Arc;
        result.geometry =
            SArcEntity{center, radius, normalizedAngle(start_angle + boundaries[index]),
...
void SCadViewport::drawTrimPreview(QPainter& painter)
{
    ...
    {
        if (hovered && (hovered->type == SEntityType::Line ||
                        hovered->type == SEntityType::Circle || hovered->type == SEntityType::Arc))
        {
    ...
    const SEntityRecord* cutting = entityById(*m_reference_entity_id);
    const std::vector<SEntityRecord> previews =
