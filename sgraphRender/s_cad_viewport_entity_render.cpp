...
QFont entityTextFont(const SCadDocument* document, const QString& style_name, double height,
                     double zoom, const QFont& fallback)
{
    QFont font = fallback;
    if (document)
    {
        if (const STextStyleRecord* style = document->textStyle(style_name))
        {
...
void SCadViewport::drawEntityGeometry(QPainter& painter, const SEntityRecord& entity,
                                      const QColor& fill_color)
{
    ...
    else if (entity.type == SEntityType::Arc)
    {
        ...
        double span_angle = arc.is_clockwise ? arc.start_angle - arc.end_angle
    ...
    else if (entity.type == SEntityType::Polyline)
    {
        ...
        for (std::size_t index = 0; index < segment_count; ++index)
        {
            ...
            const std::vector<SPoint2d> outline = polylineSegmentOutline(polyline, index);
            if (!outline.empty())
            {
                QPolygonF polygon;
                for (const SPoint2d& point : outline)
                {
            ...
            {
                SEntityRecord arc_entity = entity;
                arc_entity.type = SEntityType::Arc;
    ...
    else if (entity.type == SEntityType::Text)
    {
        ...
        double offset_x = 0.0;
        if (text.horizontal_alignment == STextHorizontalAlignment::Center)
        {
            offset_x = -text_bounds.width() * 0.5;
        }
        else if (text.horizontal_alignment == STextHorizontalAlignment::Right)
        {
            offset_x = -text_bounds.width();
        }
        double offset_y = 0.0;
        if (text.vertical_alignment == STextVerticalAlignment::Top)
        {
            offset_y = text_bounds.height();
        }
        else if (text.vertical_alignment == STextVerticalAlignment::Middle)
        {
    ...
    {
        drawDimensionEntity(painter, std::get<SLinearDimensionEntity>(entity.geometry));
    }
    else if (entity.type == SEntityType::Hatch)
    {
        drawHatchEntity(painter, std::get<SHatchEntity>(entity.geometry), fill_color);
    }
