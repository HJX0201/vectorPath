...
bool infiniteLineIntersection(const SLineEntity& first_line, const SLineEntity& second_line,
...
bool pickedDirection(const SLineEntity& line, const SPoint2d& intersection,
                     const SPoint2d& pick_point, SPoint2d& direction, SPoint2d& retained_point)
{
                          ...
                          SEntityRecord& arc_result)
{
    ...
    const SPoint2d first_tangent{intersection.x + (first_direction.x * tangent_distance),
                                 intersection.y + (first_direction.y * tangent_distance)};
    const SPoint2d second_tangent{intersection.x + (second_direction.x * tangent_distance),
                                  intersection.y + (second_direction.y * tangent_distance)};
    const SPoint2d center{intersection.x + ((bisector_x / bisector_length) * center_distance),
                          intersection.y + ((bisector_y / bisector_length) * center_distance)};
    first_result = first_source;
    first_result.geometry = SLineEntity{first_retained, first_tangent};
    second_result = second_source;
    second_result.geometry = SLineEntity{second_retained, second_tangent};
    ...
    arc_result.type = SEntityType::Arc;
    arc_result.geometry = SArcEntity{center, radius, start_angle, end_angle};
    return true;
}

void SCadViewport::setFilletRadius(double radius)
{
...
double SCadViewport::filletRadius() const noexcept
{
    return m_fillet_radius;
}

void SCadViewport::acceptFilletPoint(const SPoint2d& world_point)
{
    ...
    if (!picked_entity ||
        (picked_entity->type != SEntityType::Line && picked_entity->type != SEntityType::Circle &&
         picked_entity->type != SEntityType::Arc && picked_entity->type != SEntityType::Polyline))
    {
    ...
    const SEntityId arc_id = transaction->addEntityCopy(std::move(arc_result));
    transaction->commit();
...
void SCadViewport::drawFilletPreview(QPainter& painter)
{
        ...
        if (hovered && hovered->type == SEntityType::Polyline)
        {
            ...
            if (filletedPolylineEntity(*hovered, m_fillet_radius, preview))
            {
                ...
                drawEntityGeometry(painter, preview, QColor(92, 214, 255, 72));
                 ...
                 (hovered->type == SEntityType::Line || hovered->type == SEntityType::Circle ||
                  hovered->type == SEntityType::Arc))
        {
