...
bool SCadViewport::submitCommandKeyword(const QString& keyword)
{
    ...
    if (m_tool_mode == SToolMode::Leader && isKeyword(normalized_keyword, "UNDO", "U"))
    {
    ...
    if (m_tool_mode == SToolMode::Circle)
    {
        ...
        {
            setCircleConstruction(SCircleConstruction::TangentTangentTangent);
            return true;
        }
        if (normalized_keyword == QLatin1String("2P"))
        {
            setCircleConstruction(SCircleConstruction::TwoPoint);
            return true;
        }
        if (normalized_keyword == QLatin1String("3P"))
        {
            setCircleConstruction(SCircleConstruction::ThreePoint);
            return true;
        }
        if (isKeyword(normalized_keyword, "DIAMETER", "D"))
        {
            setCircleConstruction(SCircleConstruction::CenterDiameter);
            return true;
        }
        if (isKeyword(normalized_keyword, "RADIUS", "R"))
        {
            setCircleConstruction(SCircleConstruction::CenterRadius);
            return true;
        }
    }
    ...
    if (m_tool_mode == SToolMode::Arc)
    {
        const bool is_parameter_method =
            m_arc_construction == SArcConstruction::StartCenterAngle ||
            m_arc_construction == SArcConstruction::CenterStartAngle ||
            m_arc_construction == SArcConstruction::StartEndAngle ||
            m_arc_construction == SArcConstruction::StartEndDirection ||
            m_arc_construction == SArcConstruction::StartEndRadius;
        ...
        if (normalized_keyword == QLatin1String("3P"))
        {
            setArcConstruction(SArcConstruction::ThreePoint);
            return true;
        }
        if (normalized_keyword == QLatin1String("CSE") ||
            normalized_keyword == QLatin1String("CENTER"))
        {
            setArcConstruction(SArcConstruction::CenterStartEnd);
            return true;
        }
        if (normalized_keyword == QLatin1String("SCE"))
        {
            setArcConstruction(SArcConstruction::StartCenterEnd);
            return true;
        }
        const QStringList arc_parts =
            normalized_keyword.split(QLatin1Char(' '), QString::SkipEmptyParts);
        SArcConstruction construction = SArcConstruction::ThreePoint;
        bool is_parameter_keyword = true;
        if (!arc_parts.empty() && arc_parts.front() == QLatin1String("SCA"))
        {
            construction = SArcConstruction::StartCenterAngle;
        }
    ...
    if (m_tool_mode == SToolMode::PolylineEdit && handlePolylineEditKeyword(normalized_keyword))
    {
        return true;
    }

    if (m_tool_mode == SToolMode::SplineEdit && handleSplineEditKeyword(normalized_keyword))
    {
        return true;
    }
    ...
    if (m_tool_mode == SToolMode::Line)
    {
        ...
        if (isKeyword(normalized_keyword, "UNDO", "U"))
        {
            ...
            const SEntityRecord& last_entity = m_document->entities().back();
            if (last_entity.type != SEntityType::Line)
            {
                emit commandMessage(tr("LINE 没有可放弃的线段。"));
                return true;
            }
            const auto& last_line = std::get<SLineEntity>(last_entity.geometry);
    ...
    if ((m_tool_mode == SToolMode::Arc || m_tool_mode == SToolMode::Circle ||
         m_tool_mode == SToolMode::Spline || dimensionTypeForToolMode(m_tool_mode)) &&
    ...
    if (m_tool_mode == SToolMode::Join && isKeyword(normalized_keyword, "DONE", "ENTER"))
    {
        completeJoin();
        return true;
    }
