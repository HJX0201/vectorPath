...
VpPoint2d midpoint(const VpPoint2d& first, const VpPoint2d& second)
{
    return {(first.x + second.x) * 0.5, (first.y + second.y) * 0.5};
}

double crossProduct(const VpPoint2d& first, const VpPoint2d& second)
{
    return (first.x * second.y) - (first.y * second.x);
}

bool finitePoint(const VpPoint2d& point)
{
...
{
    const VpPoint2d chord{end_point.x - start_point.x, end_point.y - start_point.y};
    ...
    const VpPoint2d center_point = midpoint(start_point, end_point);
    ...
    result = normalized_side > 0.0 ? VpArcEntity{center, radius, start_angle, end_angle}
                                   : VpArcEntity{center, radius, end_angle, start_angle};
    return true;
}
                              ...
                              double direction_angle, VpArcEntity& result) noexcept
{
    ...
    const VpPoint2d left_normal{-std::sin(direction_radians), std::cos(direction_radians)};
    ...
    const VpPoint2d center{start_point.x + (left_normal.x * signed_radius),
                           start_point.y + (left_normal.y * signed_radius)};
    const double radius = std::abs(signed_radius);
    const double start_angle = entityAngleDegrees(center, start_point);
    const double end_angle = entityAngleDegrees(center, end_point);
    result = signed_radius > 0.0 ? VpArcEntity{center, radius, start_angle, end_angle}
                                 : VpArcEntity{center, radius, end_angle, start_angle};
                          ...
                          double included_angle, VpArcEntity& result) noexcept
{
    ...
    const VpPoint2d center_point = midpoint(start_point, end_point);
    const VpPoint2d center{center_point.x - (chord.y * center_offset / chord_length),
                           center_point.y + (chord.x * center_offset / chord_length)};
    const double radius = chord_length / (2.0 * std::abs(std::sin(half_angle_radians)));
    const double start_angle = entityAngleDegrees(center, start_point);
    const double end_angle = entityAngleDegrees(center, end_point);
    result = included_angle > 0.0 ? VpArcEntity{center, radius, start_angle, end_angle}
                                  : VpArcEntity{center, radius, end_angle, start_angle};
    return std::isfinite(radius) && radius > kEpsilon;
}
