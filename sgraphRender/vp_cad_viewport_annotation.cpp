...
void VpCadViewport::setPendingMText(QString rich_text, double width, double height,
...
void VpCadViewport::setPendingLeader(QString text, double text_height, double arrow_size)
{
...
void VpCadViewport::acceptMTextPoint(const VpPoint2d& world_point)
{
...
void VpCadViewport::acceptLeaderPoint(const VpPoint2d& world_point)
{
...
void VpCadViewport::drawAnnotationPreview(QPainter& painter)
{
    if (m_tool_mode == VpToolMode::MText && !m_pending_mtext.rich_text.trimmed().isEmpty())
    {
        VpEntityRecord preview;
        preview.type = VpEntityType::MText;
    ...
    if (m_tool_mode != VpToolMode::Leader || m_pending_leader_text.trimmed().isEmpty())
    {
        return;
    }
    ...
    VpEntityRecord preview;
    preview.type = VpEntityType::Leader;
    preview.geometry = VpLeaderEntity{std::move(preview_points), m_pending_leader_text,
