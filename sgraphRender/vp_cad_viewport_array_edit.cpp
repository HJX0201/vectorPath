...
const VpAssociativeArrayData* selectedArrayData(const VpCadDocument* document,
                                                const std::vector<VpEntityId>& selected_ids)
{
    if (!document)
    {
        return nullptr;
    }
    for (VpEntityId selected_id : selected_ids)
    {
...
bool VpCadViewport::editSelectedRectangularArray(int column_count, int row_count,
                                                 double column_spacing, double row_spacing)
{
    const VpAssociativeArrayData* selected_data =
        selectedArrayData(m_document, m_selected_entity_ids);
    if (!selected_data || selected_data->array_type != VpArrayType::Rectangular ||
    ...
    VpAssociativeArrayData parameters = *selected_data;
...
bool VpCadViewport::editSelectedPolarArray(int item_count, double fill_angle)
{
    const VpAssociativeArrayData* selected_data =
        selectedArrayData(m_document, m_selected_entity_ids);
    if (!selected_data || selected_data->array_type != VpArrayType::Polar || item_count < 2 ||
...
bool VpCadViewport::editSelectedPathArray(int item_count, bool align_to_path)
{
    const VpAssociativeArrayData* selected_data =
        selectedArrayData(m_document, m_selected_entity_ids);
    if (!selected_data || selected_data->array_type != VpArrayType::Path || item_count < 2 ||
        item_count > 10000)
    {
...
bool VpCadViewport::editAssociativeArray(VpAssociativeArrayData parameters)
{
...
void VpCadViewport::acceptArrayEditPoint(const VpPoint2d& world_point)
{
...
void VpCadViewport::drawArrayEditPreview(QPainter& painter)
{
    ...
    const VpArrayId array_id = hovered->associative_array->array_id;
    painter.save();
