...
std::vector<VpPathSegment> pathSegments(const VpEntityRecord& path, bool& is_closed,
                                        double& total_length)
{
    std::vector<VpPoint2d> vertices;
    is_closed = false;
    if (path.type == VpEntityType::Line)
    {
        const auto& line = std::get<VpLineEntity>(path.geometry);
        vertices = {line.start_point, line.end_point};
    }
    else if (path.type == VpEntityType::Polyline)
    {
        const auto& polyline = std::get<VpPolylineEntity>(path.geometry);
    ...
    std::vector<VpPathSegment> segments;
    total_length = 0.0;
    const auto add_segment = [&](const VpPoint2d& start_point, const VpPoint2d& end_point)
    {
...
VpPoint2d pointOnPath(const std::vector<VpPathSegment>& segments, double target_distance,
                      double& tangent_angle)
{
    const auto iterator =
        std::find_if(segments.begin(), segments.end(),
                     [target_distance](const VpPathSegment& segment)
                     {
                         return target_distance <= segment.start_distance + segment.length + 1.0e-9;
                     });
    const VpPathSegment& segment = iterator == segments.end() ? segments.back() : *iterator;
                                              ...
                                              bool align_to_path)
{
    ...
    const std::vector<VpPathSegment> segments = pathSegments(path, is_closed, total_length);
    ...
    for (int item = 0; item < item_count; ++item)
    {
        ...
        const VpPoint2d item_point = pointOnPath(segments, item * spacing, tangent_angle);
        for (const VpEntityRecord& source : sources)
        {
            VpEntityRecord result = align_to_path ? rotatedEntity(source, source_base_point,
...
void VpCadViewport::setPathArrayParameters(int item_count, bool align_to_path)
{
...
void VpCadViewport::acceptPathArrayPoint(const VpPoint2d& world_point)
{
    ...
    const std::optional<VpEntityId> path_id = entityAt(world_point);
    const VpEntityRecord* path = path_id ? entityById(*path_id) : nullptr;
    if (!path || (path->type != VpEntityType::Line && path->type != VpEntityType::Polyline) ||
...
void VpCadViewport::drawPathArrayPreview(QPainter& painter)
{
    if (!m_first_point || m_selected_entity_ids.empty())
    {
        return;
    }
    const std::optional<VpEntityId> path_id = entityAt(m_cursor_world);
    const VpEntityRecord* path = path_id ? entityById(*path_id) : nullptr;
    if (!path || (path->type != VpEntityType::Line && path->type != VpEntityType::Polyline) ||
        std::find(m_selected_entity_ids.begin(), m_selected_entity_ids.end(), path->id) !=
            m_selected_entity_ids.end())
    ...
    const std::vector<VpEntityRecord> previews = pathArrayEntities(
