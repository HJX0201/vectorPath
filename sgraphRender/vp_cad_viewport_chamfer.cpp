...
bool lineCornerRays(const VpEntityRecord& first_source, const VpEntityRecord& second_source,
                    const VpPoint2d& first_pick, const VpPoint2d& second_pick,
                    VpPoint2d& intersection, VpPoint2d& first_direction,
                    VpPoint2d& second_direction, VpPoint2d& first_retained,
                    VpPoint2d& second_retained)
{
    ...
    const auto orient_direction = [&intersection](const VpLineEntity& line,
                                                  const VpPoint2d& pick_point, VpPoint2d& direction,
                                                  VpPoint2d& retained)
    {
                           ...
                           VpEntityRecord& chamfer_result)
{
    ...
    const VpPoint2d first_chamfer{intersection.x + (first_direction.x * first_distance),
                                  intersection.y + (first_direction.y * first_distance)};
    const VpPoint2d second_chamfer{intersection.x + (second_direction.x * second_distance),
                                   intersection.y + (second_direction.y * second_distance)};
    first_result = first_source;
    first_result.geometry = VpLineEntity{first_retained, first_chamfer};
    second_result = second_source;
    second_result.geometry = VpLineEntity{second_retained, second_chamfer};
    chamfer_result = first_source;
    chamfer_result.geometry = VpLineEntity{first_chamfer, second_chamfer};
...
{
    if (source.type != VpEntityType::Polyline || first_distance <= 1.0e-9 ||
    ...
    {
        const VpPoint2d& previous = polyline.vertices[(index + vertex_count - 1) % vertex_count];
        const VpPoint2d& corner = polyline.vertices[index];
        const VpPoint2d& next = polyline.vertices[(index + 1) % vertex_count];
    ...
    result = source;
    result.geometry = VpPolylineEntity{std::move(vertices), polyline.is_closed};
    return true;
}

void VpCadViewport::setChamferDistances(double first_distance, double second_distance)
{
...
void VpCadViewport::acceptChamferPoint(const VpPoint2d& world_point)
{
    ...
    if (!picked_entity ||
        (picked_entity->type != VpEntityType::Line && picked_entity->type != VpEntityType::Arc &&
    ...
    const VpEntityId chamfer_id = transaction->addEntityCopy(std::move(chamfer_result));
    transaction->commit();
...
void VpCadViewport::drawChamferPreview(QPainter& painter)
{
        ...
        if (hovered && hovered->type == VpEntityType::Polyline)
        {
                                        ...
                                        m_chamfer_second_distance, preview))
            {
                ...
                drawEntityGeometry(painter, preview, QColor(92, 214, 255, 72));
                 ...
                 (hovered->type == VpEntityType::Line || hovered->type == VpEntityType::Arc))
        {
