...
std::vector<std::pair<VpPoint2d, double>>
infiniteLineCircleIntersections(const VpLineEntity& line, const VpPoint2d& center, double radius)
{
    std::vector<std::pair<VpPoint2d, double>> results;
...
bool boundaryAcceptsPoint(const VpEntityRecord& boundary, const VpPoint2d& point)
{
    if (boundary.type != VpEntityType::Arc)
    {
        return true;
    }
    const auto& arc = std::get<VpArcEntity>(boundary.geometry);
    return angleOnArc(entityAngleDegrees(arc.center, point), arc);
}

std::vector<VpPoint2d> arcBoundaryIntersections(const VpArcEntity& target_arc,
                                                const VpEntityRecord& boundary)
{
    std::vector<VpPoint2d> points;
    if (boundary.type == VpEntityType::Line)
    {
        for (const auto& intersection : infiniteLineCircleIntersections(
                 std::get<VpLineEntity>(boundary.geometry), target_arc.center, target_arc.radius))
        {
    ...
    else if (boundary.type == VpEntityType::Circle || boundary.type == VpEntityType::Arc)
    {
        const VpPoint2d center = boundary.type == VpEntityType::Circle
                                     ? std::get<VpCircleEntity>(boundary.geometry).center
                                     : std::get<VpArcEntity>(boundary.geometry).center;
        const double radius = boundary.type == VpEntityType::Circle
                                  ? std::get<VpCircleEntity>(boundary.geometry).radius
                                  : std::get<VpArcEntity>(boundary.geometry).radius;
                    ...
                    const VpPoint2d& pick_point, VpEntityRecord& result)
{
    if (boundary_entity.id == target_entity.id ||
        (boundary_entity.type != VpEntityType::Line &&
         boundary_entity.type != VpEntityType::Circle && boundary_entity.type != VpEntityType::Arc))
    {
        return false;
    }
    if (target_entity.type == VpEntityType::Line)
    {
        ...
        std::vector<std::pair<VpPoint2d, double>> candidates;
        if (boundary_entity.type == VpEntityType::Line)
        {
        ...
        {
            const VpPoint2d center = boundary_entity.type == VpEntityType::Circle
                                         ? std::get<VpCircleEntity>(boundary_entity.geometry).center
                                         : std::get<VpArcEntity>(boundary_entity.geometry).center;
            const double radius = boundary_entity.type == VpEntityType::Circle
                                      ? std::get<VpCircleEntity>(boundary_entity.geometry).radius
                                      : std::get<VpArcEntity>(boundary_entity.geometry).radius;
        ...
        std::optional<VpPoint2d> best_point;
    ...
    if (target_entity.type == VpEntityType::Polyline)
    {
        ...
        VpEntityRecord endpoint_segment = target_entity;
        VpArcEntity source_arc;
        if (bulgeArc(polyline.vertices[segment_index], polyline.vertices[segment_index + 1],
                     segment_bulge, source_arc))
        {
            endpoint_segment.type = VpEntityType::Arc;
            endpoint_segment.geometry = source_arc;
        }
        else
        {
            endpoint_segment.type = VpEntityType::Line;
            endpoint_segment.geometry = VpLineEntity{polyline.vertices[segment_index],
                                                     polyline.vertices[segment_index + 1]};
        }
        VpEntityRecord extended_segment;
        if (!extendedEntity(boundary_entity, endpoint_segment, pick_point, extended_segment))
        {
            return false;
        }
        result = target_entity;
        auto& result_polyline = std::get<VpPolylineEntity>(result.geometry);
        if (extended_segment.type == VpEntityType::Line)
        {
            const auto& result_line = std::get<VpLineEntity>(extended_segment.geometry);
        ...
        {
            const auto& result_arc = std::get<VpArcEntity>(extended_segment.geometry);
            const VpPoint2d arc_start = arcPoint(result_arc, result_arc.start_angle);
            const VpPoint2d arc_end = arcPoint(result_arc, result_arc.end_angle);
            if (extend_start)
            {
    ...
    if (target_entity.type != VpEntityType::Arc)
    {
        return false;
    }
    const auto& target_arc = std::get<VpArcEntity>(target_entity.geometry);
    const std::vector<VpPoint2d> candidates = arcBoundaryIntersections(target_arc, boundary_entity);
    ...
    for (const VpPoint2d& candidate : candidates)
    {
