...
bool joinedArcEntity(const std::vector<VpEntityRecord>& sources, double tolerance,
                     VpEntityRecord& result)
{
    ...
    const auto& reference = std::get<VpArcEntity>(sources.front().geometry);
    ...
    {
        const auto& candidate_arc = std::get<VpArcEntity>(sources[candidate].geometry);
        const VpPoint2d candidate_start = arcPoint(candidate_arc, candidate_arc.start_angle);
        bool has_predecessor = false;
        for (std::size_t other = 0; other < sources.size(); ++other)
        {
            const auto& other_arc = std::get<VpArcEntity>(sources[other].geometry);
            if (other != candidate &&
    ...
    while (order.size() < sources.size())
    {
        ...
        const auto& current_arc = std::get<VpArcEntity>(sources[current_index].geometry);
        const VpPoint2d current_end = arcPoint(current_arc, current_arc.end_angle);
    ...
    {
        const auto& arc = std::get<VpArcEntity>(sources[index].geometry);
    ...
    {
        result.type = VpEntityType::Circle;
        result.geometry = VpCircleEntity{reference.center, reference.radius};
        return true;
    }
    const auto& first_arc = std::get<VpArcEntity>(sources[order.front()].geometry);
    const auto& last_arc = std::get<VpArcEntity>(sources[order.back()].geometry);
    result.geometry =
        VpArcEntity{reference.center, reference.radius, first_arc.start_angle, last_arc.end_angle};
    return !is_closed_chain;
}

struct VpPolylinePath
{
...
VpPolylinePath reversedPath(const VpPolylinePath& source)
{
    VpPolylinePath result;
...
void appendPath(VpPolylinePath& destination, const VpPolylinePath& addition)
{
...
bool joinedLinearEntity(const std::vector<VpEntityRecord>& sources, double tolerance,
                        VpEntityRecord& result)
{
    if (sources.size() < 2 || tolerance <= 0.0)
    {
    ...
    std::vector<VpPolylinePath> paths;
    ...
    for (const VpEntityRecord& source : sources)
    {
        ...
        VpPolylinePath path{polyline.vertices, polyline.bulges, polyline.start_widths,
                            polyline.end_widths};
    ...
    VpPolylinePath joined = paths.front();
    ...
    while (used_count < paths.size())
    {
        ...
        for (std::size_t index = 1; index < paths.size(); ++index)
        {
            ...
            VpPolylinePath candidate = paths[index];
    ...
    result.geometry =
        VpPolylineEntity{std::move(joined.vertices), is_closed, std::move(joined.bulges),
