...
void VpCadViewport::acceptPoint(const VpPoint2d& world_point)
{
    if (!m_document)
    {
        return;
    ...
    if (m_tool_mode == VpToolMode::Erase)
    {
        selectAt(world_point);
    ...
    if (m_tool_mode == VpToolMode::PolylineEdit)
    {
        acceptPolylineEditPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::SplineEdit)
    {
        acceptSplineEditPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::MText)
    {
        acceptMTextPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::Leader)
    {
        acceptLeaderPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::Lengthen)
    {
        acceptLengthenPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::Trim)
    {
        ...
        {
            if (picked_entity && (picked_entity->type == VpEntityType::Line ||
                                  picked_entity->type == VpEntityType::Circle ||
                                  picked_entity->type == VpEntityType::Arc))
            {
        ...
        const VpEntityRecord* cutting_entity = entityById(*m_reference_entity_id);
        if (!cutting_entity || !picked_entity ||
            (picked_entity->type != VpEntityType::Line &&
             picked_entity->type != VpEntityType::Circle &&
             picked_entity->type != VpEntityType::Arc &&
             picked_entity->type != VpEntityType::Polyline) ||
            picked_entity->id == cutting_entity->id)
        {
            emit commandMessage(tr("TRIM 请选择与剪切边相交的直线、圆、圆弧或多段线。"));
            return;
        }
        std::vector<VpEntityRecord> replacements =
    ...
    if (m_tool_mode == VpToolMode::Extend)
    {
        const std::optional<VpEntityId> picked_id = entityAt(world_point);
        const VpEntityRecord* picked_entity = picked_id ? entityById(*picked_id) : nullptr;
        if (!m_reference_entity_id)
        ...
        const VpEntityRecord* boundary_entity = entityById(*m_reference_entity_id);
    ...
    if (m_tool_mode == VpToolMode::Break)
    {
        if (!m_reference_entity_id)
        {
            const std::optional<VpEntityId> picked_id = entityAt(world_point);
        ...
        std::vector<VpEntityRecord> pieces =
            brokenEntityParts(*source, *m_first_point, world_point);
        if (pieces.empty())
        {
        ...
        std::vector<VpEntityId> piece_ids{source->id};
    ...
    if (m_tool_mode == VpToolMode::Join)
    {
        const std::optional<VpEntityId> picked_id = entityAt(world_point);
        const VpEntityRecord* picked_entity = picked_id ? entityById(*picked_id) : nullptr;
        if (!picked_entity || (picked_entity->type != VpEntityType::Line &&
        ...
        if (!m_selected_entity_ids.empty())
        {
            const VpEntityRecord* first_source = entityById(m_selected_entity_ids.front());
            const bool first_is_linear =
                first_source && (first_source->type == VpEntityType::Line ||
                                 first_source->type == VpEntityType::Polyline);
            const bool picked_is_linear = picked_entity->type == VpEntityType::Line ||
                                          picked_entity->type == VpEntityType::Polyline;
    ...
    if (m_tool_mode == VpToolMode::Explode)
    {
        const std::optional<VpEntityId> picked_id = entityAt(world_point);
        const VpEntityRecord* source = picked_id ? entityById(*picked_id) : nullptr;
        if (!source)
        {
            emit commandMessage(tr("EXPLODE 未找到可分解实体。"));
            return;
        }
        std::vector<VpEntityRecord> parts = explodedEntityParts(*source);
        if (parts.empty())
        {
        ...
        std::vector<VpEntityId> part_ids{source->id};
    ...
    if (m_tool_mode == VpToolMode::Stretch)
    {
    ...
    if (m_tool_mode == VpToolMode::Fillet)
    {
        acceptFilletPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::Chamfer)
    {
        acceptChamferPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::Blend)
    {
        acceptBlendPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::Align)
    {
        acceptAlignPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::ArrayRect)
    {
        acceptRectangularArrayPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::ArrayPolar)
    {
        acceptPolarArrayPoint(world_point);
        return;
    }
    if (m_tool_mode == VpToolMode::ArrayPath)
    {
    ...
    if (m_tool_mode == VpToolMode::Move || m_tool_mode == VpToolMode::Copy ||
        m_tool_mode == VpToolMode::Rotate || m_tool_mode == VpToolMode::Scale ||
        m_tool_mode == VpToolMode::Mirror || m_tool_mode == VpToolMode::Offset)
    {
        if (!m_selected_entity_id)
        {
            ...
            if (m_selected_entity_id)
            {
                if (m_tool_mode == VpToolMode::Offset)
                {
                    emit commandMessage(tr("OFFSET 指定通过点："));
                }
                else if (m_tool_mode == VpToolMode::Mirror)
                {
                    emit commandMessage(tr("MIRROR 指定镜像线第一点："));
                }
                else if (m_tool_mode == VpToolMode::Scale)
                {
                    emit commandMessage(tr("SCALE 指定基点："));
                }
                else if (m_tool_mode == VpToolMode::Copy)
                {
                    emit commandMessage(tr("COPY 指定基点："));
                }
                else
                {
                    emit commandMessage(m_tool_mode == VpToolMode::Move ? tr("MOVE 指定基点：")
                                                                        : tr("ROTATE 指定基点："));
                }
            }
            return;
        ...
        {
            m_first_point = world_point;
            if (m_tool_mode == VpToolMode::Mirror)
            {
            ...
            {
                emit commandMessage(m_tool_mode == VpToolMode::Move ? tr("MOVE 指定第二点：")
                                                                    : tr("ROTATE 指定旋转方向："));
            }
            update();
            return;
        }
        if (m_tool_mode == VpToolMode::Scale && m_input_points.empty())
        {
        ...
        if (m_tool_mode == VpToolMode::Move)
        {
        ...
        else if (m_tool_mode == VpToolMode::Rotate)
        {
    ...
    if (m_tool_mode == VpToolMode::Ellipse)
    {
        acceptEllipsePoint(world_point);
        return;
    }

    if (m_tool_mode == VpToolMode::Spline)
    {
    ...
    if (m_tool_mode == VpToolMode::StandardShape)
    {
        acceptStandardShapePoint(world_point);
        return;
    }

    if (m_tool_mode == VpToolMode::Text)
    {
    ...
    if (m_tool_mode == VpToolMode::Hatch)
    {
        const std::optional<VpEntityId> boundary_id = entityAt(world_point);
    ...
    if (!m_first_point.has_value())
    {
        ...
        else if (m_tool_mode == VpToolMode::Rectangle)
        {
    ...
    auto transaction = m_document->beginTransaction(
        m_tool_mode == VpToolMode::Line ? tr("创建直线") : tr("创建矩形"));
    if (m_tool_mode == VpToolMode::Line)
    {
