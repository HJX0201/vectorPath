...
void VpCadViewport::completeMove(const VpPoint2d& destination)
{
...
void VpCadViewport::completeCopy(const VpPoint2d& destination)
{
    std::vector<VpEntityId> copied_ids;
    if (!m_selected_entity_ids.empty() && m_first_point)
    {
        const double delta_x = destination.x - m_first_point->x;
        const double delta_y = destination.y - m_first_point->y;
...
void VpCadViewport::completeRotate(const VpPoint2d& destination)
{
    if (!m_selected_entity_ids.empty() && m_first_point)
    {
        ...
        for (VpEntityId entity_id : m_selected_entity_ids)
        {
            if (const VpEntityRecord* source = entityById(entity_id))
            {
                VpEntityRecord replacement = rotatedEntity(*source, *m_first_point, angle);
...
void VpCadViewport::completeScale(const VpPoint2d& destination)
{
...
void VpCadViewport::completeMirror(const VpPoint2d& axis_end)
{
    std::vector<VpEntityId> mirrored_ids;
    if (!m_selected_entity_ids.empty() && m_first_point &&
...
void VpCadViewport::completeOffset(const VpPoint2d& through_point)
{
    ...
    transaction->commit();
    emit commandMessage(source->type == VpEntityType::Spline
                            ? tr("OFFSET 已将样条的偏移结果转换为高精度多段线。指定通过点：")
                            : tr("OFFSET 指定通过点："));
    update();
}

void VpCadViewport::completeJoin()
{
    ...
    for (VpEntityId entity_id : m_selected_entity_ids)
    {
        ...
        if (!source || (source->type != VpEntityType::Line && source->type != VpEntityType::Arc &&
    ...
    const VpEntityId result_id = sources.front().id;
...
void VpCadViewport::completePolyline(bool is_closed)
{
