...
void VpCadViewport::drawGrid(QPainter& painter)
{
    const double spacing = adaptiveGridSpacing();
    const VpGridBasis basis = draftingGridBasis(m_drafting_state.gridRotation());
    const VpPoint2d center = screenToWorld(QPointF(width() * 0.5, height() * 0.5));
    ...
    for (int offset = -line_count; offset <= line_count; ++offset)
    {
        ...
        const VpPoint2d first_origin{basis.first_axis.x * first_distance,
                                     basis.first_axis.y * first_distance};
        const VpPoint2d first_start{first_origin.x - (basis.second_axis.x * guide_length),
                                    first_origin.y - (basis.second_axis.y * guide_length)};
        const VpPoint2d first_end{first_origin.x + (basis.second_axis.x * guide_length),
                                  first_origin.y + (basis.second_axis.y * guide_length)};
        ...
        const VpPoint2d second_origin{basis.second_axis.x * second_distance,
                                      basis.second_axis.y * second_distance};
        const VpPoint2d second_start{second_origin.x - (basis.first_axis.x * guide_length),
                                     second_origin.y - (basis.first_axis.y * guide_length)};
        const VpPoint2d second_end{second_origin.x + (basis.first_axis.x * guide_length),
                                   second_origin.y + (basis.first_axis.y * guide_length)};
...
void VpCadViewport::drawNavigationOverlay(QPainter& painter)
{
    painter.save();
