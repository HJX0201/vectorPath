...
void VpCadViewport::showPolylineEditPrompt()
{
...
void VpCadViewport::acceptPolylineEditPoint(const VpPoint2d& world_point)
{
    ...
    if (!picked_entity || picked_entity->type != VpEntityType::Polyline)
    {
...
void VpCadViewport::completePolylineEditJoin()
{
    ...
    for (VpEntityId entity_id : m_selected_entity_ids)
    {
        ...
        if (!source || source->type != VpEntityType::Polyline)
        {
    ...
    if (!joinedEntity(sources, 1.0e-6, joined) || joined.type != VpEntityType::Polyline)
    {
...
bool VpCadViewport::handlePolylineEditKeyword(const QString& normalized_keyword)
{
    ...
    for (VpEntityId entity_id : m_selected_entity_ids)
    {
        ...
        const auto& source_polyline = std::get<VpPolylineEntity>(source->geometry);
