...
void VpCadViewport::showSplineEditPrompt()
{
...
void VpCadViewport::acceptSplineEditPoint(const VpPoint2d& world_point)
{
    if (!m_selected_entity_id)
    {
        ...
        if (!picked || picked->type != VpEntityType::Spline)
        {
    ...
    if (!source || source->type != VpEntityType::Spline)
    {
    ...
    if (!m_spline_edit_control_index)
    {
        const auto& control_points = std::get<VpSplineEntity>(source->geometry).control_points;
...
bool VpCadViewport::handleSplineEditKeyword(const QString& normalized_keyword)
{
...
void VpCadViewport::drawSplineEditPreview(QPainter& painter)
{
    if (!m_selected_entity_id)
    {
        return;
    ...
    const auto& control_points = std::get<VpSplineEntity>(preview.geometry).control_points;
