...
bool pointInsideWindow(const VpPoint2d& point, const VpPoint2d& first_corner,
                       const VpPoint2d& second_corner)
{
                     ...
                     VpEntityRecord& result)
{
    ...
    const auto stretch_point = [&](VpPoint2d& point)
    {
    ...
    else
    {
        ...
        {
            anchor = std::get<VpCircleEntity>(result.geometry).center;
        }
        else if (result.type == VpEntityType::Arc)
        {
            anchor = std::get<VpArcEntity>(result.geometry).center;
        }
        else if (result.type == VpEntityType::Text)
        {
            anchor = std::get<VpTextEntity>(result.geometry).position;
        }
        else
        {
            return false;
...
void VpCadViewport::drawStretchPreview(QPainter& painter)
{
    if (m_input_points.empty())
    {
        return;
    }
    const VpPoint2d opposite_corner =
...
void VpCadViewport::completeStretch(const VpPoint2d& destination)
{
