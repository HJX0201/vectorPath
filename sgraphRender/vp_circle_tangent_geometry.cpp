...
#include "vp_circle_tangent_math.h"
...
using detail::VpLinearConstraint;
using detail::VpVector3;

constexpr double kPi = 3.14159265358979323846;
constexpr double kEpsilon = 1.0e-9;

struct VpCurveSupport
{
    bool is_line = false;
    bool is_arc = false;
    VpPoint2d first;
...
bool supportFromEntity(const VpEntityRecord& entity, VpCurveSupport& support)
{
...
VpLineEquation lineEquation(const VpCurveSupport& support)
{
...
bool pointOnArc(const VpCurveSupport& support, const VpPoint2d& point)
{
    if (!support.is_arc)
    {
        return true;
...
void appendLineLineIntersections(const VpLineEquation& first, const VpLineEquation& second,
...
void appendLineCircleIntersections(const VpLineEquation& line, double line_offset,
                                   const VpPoint2d& center, double radius,
                                   std::vector<VpPoint2d>& centers)
{
    ...
    const VpPoint2d projection{center.x - (line.normal_x * signed_distance),
    ...
    const VpPoint2d direction{-line.normal_y, line.normal_x};
...
void appendCircleCircleIntersections(const VpPoint2d& first_center, double first_radius,
                                     ...
                                     std::vector<VpPoint2d>& centers)
{
    ...
    const VpPoint2d base{first_center.x + (unit_x * along), first_center.y + (unit_y * along)};
...
bool tangencyPoint(const VpCurveSupport& support, const VpPoint2d& candidate_center,
                   double candidate_radius, VpPoint2d& point)
{
    if (support.is_line)
    {
        const VpLineEquation line = lineEquation(support);
...
struct VpCurveEquation
{
    VpVector3 linear;
    double constant = 0.0;
};

std::array<VpTangentBranch, 3> contactBranches(const VpCurveSupport& support,
                                               std::size_t& branch_count)
{
    if (support.is_line)
    {
        branch_count = 2;
        return {VpTangentBranch{-1.0, 0.0, 0.0}, VpTangentBranch{1.0, 0.0, 0.0}, VpTangentBranch{}};
    }
    branch_count = 3;
    return {VpTangentBranch{0.0, 1.0, support.radius}, VpTangentBranch{0.0, -1.0, support.radius},
            VpTangentBranch{0.0, 1.0, -support.radius}};
}

VpLinearConstraint lineConstraint(const VpCurveSupport& support, const VpTangentBranch& branch)
{
    const VpLineEquation line = lineEquation(support);
    return {{line.normal_x, line.normal_y, -branch.line_sign}, line.constant};
}

VpCurveEquation curveEquation(const VpCurveSupport& support, const VpTangentBranch& branch)
{
...
VpLinearConstraint curveDifferenceConstraint(const VpCurveSupport& first_support,
                                             const VpTangentBranch& first_branch,
                                             const VpCurveSupport& second_support,
                                             const VpTangentBranch& second_branch)
{
    const VpCurveEquation first = curveEquation(first_support, first_branch);
    const VpCurveEquation second = curveEquation(second_support, second_branch);
...
bool branchSatisfied(const VpCurveSupport& support, const VpTangentBranch& branch,
                     const VpCircleEntity& candidate)
{
...
bool sameCircle(const VpCircleEntity& first, const VpCircleEntity& second)
{
...
void appendThreeEntityCandidates(const std::array<VpCurveSupport, 3>& supports,
                                 const std::array<VpTangentBranch, 3>& branches,
                                 std::vector<VpCircleEntity>& candidates)
{
    ...
    VpLinearConstraint first_constraint;
    VpLinearConstraint second_constraint;
    if (line_count == 2)
    {
    ...
    const VpCurveSupport& target = supports[target_index];
    const VpTangentBranch& target_branch = branches[target_index];
...
bool tangentCircleToTwoEntities(const VpEntityRecord& first_source,
                                ...
                                const VpPoint2d& second_pick, double radius, VpCircleEntity& result)
{
    if (!std::isfinite(radius) || radius <= kEpsilon)
    {
        return false;
    }
    VpCurveSupport first;
    VpCurveSupport second;
    ...
    {
        const VpLineEquation first_line = lineEquation(first);
        const VpLineEquation second_line = lineEquation(second);
    ...
    {
        const VpCurveSupport& line_support = first.is_line ? first : second;
        const VpCurveSupport& circle_support = first.is_line ? second : first;
        const VpLineEquation line = lineEquation(line_support);
    ...
    {
        VpPoint2d first_tangent;
        VpPoint2d second_tangent;
...
{
    std::array<VpCurveSupport, 3> supports;
    std::array<VpLineEquation, 3> lines;
    ...
    for (int first_sign : {-1, 1})
    {
        for (int second_sign : {-1, 1})
        {
            for (int third_sign : {-1, 1})
            {
                ...
                const VpPoint2d center{determinant3(x_matrix) / determinant,
                                  ...
                                  const std::array<VpPoint2d, 3>& picks, VpCircleEntity& result)
{
    ...
    std::array<std::array<VpTangentBranch, 3>, 3> branch_sets;
    ...
    double best_score = std::numeric_limits<double>::max();
    bool has_result = false;
    for (const VpCircleEntity& candidate : candidates)
    {
        double score = 0.0;
        for (std::size_t index = 0; index < supports.size(); ++index)
        {
