...
bool sourcePolyline(const VpEntityRecord& source, VpPolylineEntity& polyline)
{
    if (source.type != VpEntityType::Polyline)
    {
        return false;
    }
    polyline = std::get<VpPolylineEntity>(source.geometry);
...
bool polylineClosedStateEntity(const VpEntityRecord& source, bool is_closed, VpEntityRecord& result)
{
...
bool polylineConstantWidthEntity(const VpEntityRecord& source, double width, VpEntityRecord& result)
{
    VpPolylineEntity polyline;
