...
{
    if (source.type != VpEntityType::Polyline || radius <= 1.0e-9 || !std::isfinite(radius))
    {
        return false;
    }
    ...
    for (std::size_t index = first_corner; index < corner_end; ++index)
    {
        ...
        const VpPoint2d ray_to_previous{(previous.x - corner.x) / incoming_length,
                                        (previous.y - corner.y) / incoming_length};
        const VpPoint2d ray_to_next{(next.x - corner.x) / outgoing_length,
    ...
    const auto append = [&](const VpPoint2d& point, double bulge)
    {
    ...
    result = source;
    result.geometry = VpPolylineEntity{std::move(vertices), polyline.is_closed, std::move(bulges)};
    return true;
}
