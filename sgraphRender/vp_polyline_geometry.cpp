              ...
              VpArcEntity& result) noexcept
{
    ...
    const VpPoint2d midpoint{(start_point.x + end_point.x) * 0.5,
                             (start_point.y + end_point.y) * 0.5};
    const double center_offset = chord_length * (1.0 - (bulge * bulge)) / (4.0 * bulge);
    const VpPoint2d center{midpoint.x - ((chord_y / chord_length) * center_offset),
    ...
    result = VpArcEntity{center, radius, start_angle, end_angle};
    return true;
}
                                              ...
                                              std::size_t segment_index, int arc_segment_count)
{
    ...
    const VpPoint2d& end_point = polyline.vertices[(segment_index + 1) % vertex_count];
    std::vector<VpPoint2d> center_points;
    const double bulge =
    ...
    std::vector<VpPoint2d> left_points;
    std::vector<VpPoint2d> right_points;
    ...
    {
        const VpPoint2d& previous = center_points[index == 0 ? 0 : index - 1];
        const VpPoint2d& next = center_points[index + 1 < center_points.size() ? index + 1 : index];
    ...
    std::vector<VpPoint2d> outline = std::move(left_points);
