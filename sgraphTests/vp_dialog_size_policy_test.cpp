...
class VpDialogSizePolicyTest final : public QObject
{
    Q_OBJECT
...
void VpDialogSizePolicyTest::givesCustomDialogsAUsableMinimum()
{
    VpDialog dialog;
...
void VpDialogSizePolicyTest::givesInputDialogsAUsableMinimum()
{
    ...
    const QString text =
        VpDialogService::getText(nullptr, QStringLiteral("创建文字"), QStringLiteral("文字内容："),
                                 QLineEdit::Normal, QString{}, &is_accepted);
...
void VpDialogSizePolicyTest::givesMessageDialogsAUsableMinimum()
{
    bool dialog_found = false;
    int minimum_width = 0;
    int minimum_height = 0;
    QTimer::singleShot(0,
                       [&]()
                       {
                           for (QWidget* widget : QApplication::topLevelWidgets())
                           {
                               ...
                               if (!message_box ||
                                   message_box->windowTitle() != QStringLiteral("提示"))
                               {
                                   continue;
                               }
                               dialog_found = true;
    ...
    const QMessageBox::StandardButton result = VpDialogService::information(
        nullptr, QStringLiteral("提示"), QStringLiteral("操作已经完成。"));
...
void VpDialogSizePolicyTest::preservesLargerExplicitMinimum()
{
    VpDialog dialog;
