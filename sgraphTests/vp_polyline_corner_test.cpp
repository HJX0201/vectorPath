...
class VpPolylineCornerTest final : public QObject
{
    Q_OBJECT
...
void VpPolylineCornerTest::polylineChamferGeometryAndUndo()
{
    ...
    open_polyline.geometry =
        VpPolylineEntity{{{0.0, 0.0}, {10.0, 0.0}, {10.0, 10.0}, {20.0, 10.0}}, false};
    VpEntityRecord result;
    QVERIFY(chamferedPolylineEntity(open_polyline, 2.0, 3.0, result));
    const auto& open_result = std::get<VpPolylineEntity>(result.geometry);
    ...
    const auto& closed_result = std::get<VpPolylineEntity>(result.geometry);
...
void VpPolylineCornerTest::polylineFilletGeometryPersistenceAndUndo()
{
    ...
    const auto& filleted = std::get<VpPolylineEntity>(result.geometry);
    ...
    appended_line.geometry = VpLineEntity{{20.0, 10.0}, {30.0, 10.0}};
    VpEntityRecord joined_result;
    QVERIFY(joinedEntity({appended_line, result}, 1.0e-6, joined_result));
    const auto& joined_polyline = std::get<VpPolylineEntity>(joined_result.geometry);
    ...
    VpEntityRecord bulged_path;
    bulged_path.id = 232;
    bulged_path.type = VpEntityType::Polyline;
    bulged_path.geometry = VpPolylineEntity{{{0.0, 0.0}, {5.0, 5.0}, {10.0, 5.0}},
                                            false,
                                            {std::tan(3.14159265358979323846 / 8.0), 0.0, 0.0}};
    VpEntityRecord extension_boundary;
    extension_boundary.id = 233;
    extension_boundary.type = VpEntityType::Line;
    extension_boundary.geometry = VpLineEntity{{-5.0, 0.0}, {-5.0, 10.0}};
    VpEntityRecord extended_path;
    QVERIFY(extendedEntity(extension_boundary, bulged_path, {0.0, 0.0}, extended_path));
    const auto& extended_polyline = std::get<VpPolylineEntity>(extended_path.geometry);
    ...
    viewport.submitWorldPoint({5.0, 0.0});
    const auto& stored = std::get<VpPolylineEntity>(document.entities().front().geometry);
    ...
    const auto& loaded = std::get<VpPolylineEntity>(loaded_document.entities().front().geometry);
    ...
    const auto& dxf_polyline = std::get<VpPolylineEntity>(dxf_document.entities().front().geometry);
    QCOMPARE(dxf_polyline.bulges.size(), stored.bulges.size());
    QVERIFY(std::abs(dxf_polyline.bulges[1] - stored.bulges[1]) < 1.0e-9);

    document.undo();
    const auto& restored = std::get<VpPolylineEntity>(document.entities().front().geometry);
...
void VpPolylineCornerTest::bulgedPolylineTrimPreservesCurves()
{
    ...
    VpEntityRecord target;
    target.id = 241;
    target.type = VpEntityType::Polyline;
    target.layer_name = QStringLiteral("bulged_trim");
    target.line_width_mm = 0.7;
    target.geometry = VpPolylineEntity{
        {{-10.0, 0.0}, {-5.0, 0.0}, {5.0, 0.0}, {10.0, 0.0}}, false, {0.0, 1.0, 0.0, 0.0}};
    const std::vector<VpEntityRecord> positive_parts =
        trimmedEntityParts(cutting_line, target, {-3.5, -3.5});
    QCOMPARE(positive_parts.size(), std::size_t(2));
    const auto& positive_right = std::get<VpPolylineEntity>(positive_parts.back().geometry);
    ...
    const std::vector<VpEntityRecord> closed_parts =
        trimmedEntityParts(cutting_line, target, {-3.5, 3.5});
    QCOMPARE(closed_parts.size(), std::size_t(1));
    const auto& opened = std::get<VpPolylineEntity>(closed_parts.front().geometry);
...
void VpPolylineCornerTest::interactiveArcSegmentAndUndo()
{
    double bulge = 0.0;
...
void VpPolylineCornerTest::variableWidthPersistenceAndDxf()
{
    VpPolylineEntity geometry{{{0.0, 0.0}, {10.0, 0.0}}, false, {0.0, 0.0}, {2.0, 0.0}, {4.0, 0.0}};
    const std::vector<VpPoint2d> outline = polylineSegmentOutline(geometry, 0);
...
void VpPolylineCornerTest::polylineEditOperationsAndJoin()
{
    VpEntityRecord source;
    source.id = 401;
    source.type = VpEntityType::Polyline;
    source.geometry = VpPolylineEntity{{{0.0, 0.0}, {10.0, 0.0}, {10.0, 8.0}},
                                       false,
                                       {0.5, -0.25, 0.0},
                                       {1.0, 2.0, 0.0},
                                       {2.0, 3.0, 0.0}};
    ...
    const auto& reversed = std::get<VpPolylineEntity>(edited.geometry);
    ...
    QVERIFY(std::get<VpPolylineEntity>(edited.geometry).is_closed);
    QVERIFY(decurvedPolylineEntity(source, edited));
    const auto& decurved = std::get<VpPolylineEntity>(edited.geometry);
    ...
    viewport.setDocument(&document);
    viewport.selectAll();
    viewport.setToolMode(VpToolMode::PolylineEdit);
    QVERIFY(viewport.submitCommandKeyword(QStringLiteral("JOIN")));
    QCOMPARE(document.entities().size(), std::size_t(1));
    const auto& joined = std::get<VpPolylineEntity>(document.entities().front().geometry);
    ...
    const auto& widened = std::get<VpPolylineEntity>(document.entities().front().geometry);
    ...
    QVERIFY(std::get<VpPolylineEntity>(document.entities().front().geometry).is_closed);
    QVERIFY(viewport.submitCommandKeyword(QStringLiteral("OPEN")));
    QVERIFY(!std::get<VpPolylineEntity>(document.entities().front().geometry).is_closed);
    QVERIFY(viewport.submitCommandKeyword(QStringLiteral("REVERSE")));
    QCOMPARE(std::get<VpPolylineEntity>(document.entities().front().geometry).vertices.front().x,
             20.0);
    document.undo();
    QCOMPARE(std::get<VpPolylineEntity>(document.entities().front().geometry).vertices.front().x,
             0.0);
