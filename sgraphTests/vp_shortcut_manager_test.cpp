...
class VpShortcutManagerTest final : public QObject
{
    Q_OBJECT
...
void VpShortcutManagerTest::initTestCase()
{
    ...
    QCoreApplication::setApplicationName(QStringLiteral("vectorPathShortcutManagerTests"));
    QSettings().clear();
}

void VpShortcutManagerTest::cleanup()
{
    QSettings().clear();
}

void VpShortcutManagerTest::persistsAndRestoresDefaults()
{
    QAction first_action(QStringLiteral("直线"), this);
    QAction duplicate_action(QStringLiteral("直线工具栏"), this);
    VpShortcutManager manager;
    ...
    VpShortcutManager restored_manager;
...
void VpShortcutManagerTest::rejectsShortcutConflicts()
{
    ...
    QVERIFY(
        manager.setShortcut(QStringLiteral("file.save"), QKeySequence(QStringLiteral("Ctrl+O"))));
}

void VpShortcutManagerTest::exposesModernShortcutDialog()
{
    ...
    QTimer::singleShot(0, &window,
                       [&dialog_verified]()
                       {
                           ...
                           {
                               auto* dialog = dynamic_cast<VpShortcutDialog*>(widget);
                               if (!dialog)
                               {
                                   continue;
                               }
                               QTreeWidget* tree = dialog->findChild<QTreeWidget*>(
                                   QStringLiteral("smartShortcutTree"));
                               QKeySequenceEdit* editor = dialog->findChild<QKeySequenceEdit*>(
                                   QStringLiteral("smartShortcutSequenceEdit"));
                               dialog_verified = tree && tree->topLevelItemCount() >= 60 &&
                                                 editor && editor->isEnabled();
                               dialog->accept();
                               return;
                           }
                       });
