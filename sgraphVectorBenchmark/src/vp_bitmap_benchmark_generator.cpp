...
const QVector<QRgb>& palette()
{
    static const QVector<QRgb> colors{
        qRgba(244, 246, 250, 255), qRgba(32, 82, 149, 255),  qRgba(225, 69, 75, 255),
        qRgba(40, 155, 102, 255),  qRgba(244, 174, 66, 255), qRgba(125, 85, 171, 255),
        qRgba(36, 158, 183, 255),  qRgba(112, 73, 51, 255),  qRgba(239, 119, 178, 255),
        qRgba(93, 105, 118, 255),  qRgba(181, 203, 74, 255), qRgba(22, 33, 62, 255),
        qRgba(255, 214, 102, 255), qRgba(79, 70, 229, 255),  qRgba(15, 118, 110, 255),
        qRgba(190, 24, 93, 255)};
    return colors;
}
...
QSize caseSize(int index, bool smoke, QRandomGenerator& random)
{
    ...
    const int height =
        ratio == 0 ? width : std::max(64, width * (ratio == 1 ? 3 : 2) / (ratio == 1 ? 4 : 3));
    return {width, height};
}
...
void drawBands(QImage& image, QRandomGenerator& random)
{
    ...
    for (int index = 0; index < band_count; ++index)
    {
        ...
        const int begin =
            vertical ? image.width() * index / band_count : image.height() * index / band_count;
        const int end = vertical ? image.width() * (index + 1) / band_count
                                 : image.height() * (index + 1) / band_count;
        const QRect rectangle = vertical ? QRect(begin, 0, end - begin, image.height())
                                         : QRect(0, begin, image.width(), end - begin);
        painter.fillRect(rectangle, QColor::fromRgba(palette()[(index + 1) % 8]));
    }
}
...
void drawNested(QImage& image, QRandomGenerator& random)
{
    ...
    for (int index = 0; index < layers && rectangle.width() > 4 && rectangle.height() > 4; ++index)
    {
        painter.fillRect(rectangle, QColor::fromRgba(palette()[(index + 1) % 10]));
        const int inset =
            std::max(1, std::min(rectangle.width(), rectangle.height()) / (layers * 2 + 2));
...
{
    QPainter painter(&image);
    const QColor color = QColor::fromRgba(palette()[1 + random.bounded(8)]);
    const int branch_width = std::max(1, image.width() / 16);
    const int center_x = image.width() / 2;
    painter.fillRect(center_x - branch_width, image.height() / 6, branch_width * 2,
                     image.height() * 2 / 3, color);
    painter.fillRect(image.width() / 6, image.height() / 6, image.width() * 2 / 3, branch_width,
                     color);
    painter.fillRect(image.width() / 6, image.height() * 5 / 6 - branch_width,
                     image.width() * 2 / 3, branch_width, color);
    painter.fillRect(image.width() / 5, image.height() / 2, image.width() * 3 / 5, branch_width,
                     color);
}
...
void drawThinCorridors(QImage& image, QRandomGenerator& random)
{
    ...
    {
        const QColor color = QColor::fromRgba(palette()[1 + random.bounded(12)]);
...
void drawChecker(QImage& image, QRandomGenerator& random)
{
    ...
    for (int y = 0; y < image.height(); y += cell)
    {
        ...
        {
            painter.fillRect(x, y, cell, cell, ((x / cell + y / cell) % 2 == 0) ? first : second);
        }
    }
}
...
{
    QPainter painter(&image);
    const int count = std::min(256, 12 + image.width() * image.height() / 32768);
    for (int index = 0; index < count; ++index)
    {
        ...
        painter.fillRect(x, y, width, height, QColor::fromRgba(palette()[1 + random.bounded(15)]));
    }
}
...
void drawDisconnected(QImage& image, QRandomGenerator& random)
{
    ...
    for (int row = 0; row < rows; ++row)
    {
        for (int column = 0; column < columns; ++column)
        {
            ...
            painter.fillRect(
                column * cell_width + inset, row * cell_height + inset,
                std::max(1, cell_width - inset * 2), std::max(1, cell_height - inset * 2),
                (row + column) % 3 == 0 ? shared
                                        : QColor::fromRgba(palette()[1 + (row + column) % 12]));
        }
    }
}
...
QImage generateImage(const QSize& size, const QString& category, quint32 seed, int index)
{
...
bool writeManifest(const QString& output_directory, const std::vector<VpBitmapBenchmarkCase>& cases,
                   const VpBitmapBenchmarkOptions& options)
{
    ...
    QJsonArray entries;
    for (const VpBitmapBenchmarkCase& test_case : cases)
    {
        QJsonObject entry;
        ...
        entry.insert(QStringLiteral("seed"), static_cast<qint64>(test_case.seed));
        entry.insert(QStringLiteral("color_count"), test_case.color_count);
        entry.insert(QStringLiteral("file"),
                     QDir(output_directory).relativeFilePath(test_case.file_path));
...
generateBitmapBenchmarkCases(const VpBitmapBenchmarkOptions& options)
{
    ...
    {
        return VpResult<std::vector<VpBitmapBenchmarkCase>>::failure(
            toCoreText(QStringLiteral("无法创建测试图片目录。")));
    }
    QDir cases_directory(output.filePath(QStringLiteral("cases")));
    std::vector<VpBitmapBenchmarkCase> cases;
    ...
    {
        const quint32 case_seed = options.seed ^ (0x9E3779B9U * static_cast<quint32>(index + 1));
        const QSize size = caseSize(index, options.smoke, size_random);
        const QString category = categoryForIndex(index);
        const QString file_name = QStringLiteral("case_%1_%2_%3x%4_%5.png")
        ...
        if (image.size() != size)
        {
            ...
            {
                return VpResult<std::vector<VpBitmapBenchmarkCase>>::failure(
                    toCoreText(QStringLiteral("无法保存测试图片：%1").arg(file_path)));
            }
        }
        cases.push_back({index + 1, category, size.width(), size.height(), case_seed,
                         colorCount(image), file_path});
    }
    if (!writeManifest(options.output_directory, cases, options))
    {
        return VpResult<std::vector<VpBitmapBenchmarkCase>>::failure(
            toCoreText(QStringLiteral("无法写入测试清单 manifest.json。")));
    }
    return VpResult<std::vector<VpBitmapBenchmarkCase>>::success(std::move(cases));
}
