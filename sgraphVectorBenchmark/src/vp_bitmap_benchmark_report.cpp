...
QString speedup(double baseline, double candidate)
{
    ...
    return QString::number(baseline / candidate, 'f', 2) + QStringLiteral("×");
}
...
void writeSummaryCards(QTextStream& stream, int passed, int total, double flood_ms,
                       double serial_ms, double parallel_ms, quint64 peak_bytes)
{
    stream << "<section class=\"cards\">"
           << "<div><b>" << passed << " / " << total
           << QStringLiteral("</b><span>正确性通过</span></div>") << "<div><b>"
           << milliseconds(flood_ms) << QStringLiteral(" ms</b><span>四邻域累计</span></div>")
           << "<div><b>" << milliseconds(serial_ms)
           << QStringLiteral(" ms</b><span>拆分单线程累计</span></div>") << "<div><b>"
           << milliseconds(parallel_ms)
           << QStringLiteral(" ms</b><span>拆分多线程累计</span></div>") << "<div><b>"
           << speedup(flood_ms, parallel_ms)
           << QStringLiteral("</b><span>多线程 / 四邻域</span></div>") << "<div><b>"
           << QString::number(peak_bytes / 1024.0 / 1024.0, 'f', 1)
           << QStringLiteral(" MB</b><span>进程峰值工作集</span></div></section>");
}

void writeCategoryTable(QTextStream& stream, const QMap<QString, VpAggregate>& categories)
{
    stream << QStringLiteral("<h2>按图案类别</h2><table><thead><tr>"
    ...
    {
        const VpAggregate& value = iterator.value();
        stream << "<tr><td>" << escapeHtml(iterator.key()) << "</td><td>" << value.count
               << "</td><td>" << value.passed << "</td><td>" << milliseconds(value.flood_ms)
               << "</td><td>" << milliseconds(value.serial_ms) << "</td><td>"
               << milliseconds(value.parallel_ms) << "</td><td>"
               << speedup(value.flood_ms, value.parallel_ms) << "</td></tr>";
    }
    stream << "</tbody></table>";
}

void writeWorstTable(QTextStream& stream, QString title,
                     std::vector<const VpBitmapBenchmarkCaseResult*> values, bool by_regression)
{
    std::sort(values.begin(), values.end(),
              [by_regression](const auto* first, const auto* second)
              {
                  if (by_regression)
                  {
                      const double first_speed =
                          ...
                          std::max(0.000001, first->run_parallel.milliseconds);
                      const double second_speed =
                          second->flood_fill.milliseconds /
                          std::max(0.000001, second->run_parallel.milliseconds);
                      return first_speed < second_speed;
                  }
                  return first->run_parallel.milliseconds > second->run_parallel.milliseconds;
              });
    stream << "<h2>" << title
           << QStringLiteral("</h2><table><thead><tr>"
    ...
    for (std::size_t index = 0; index < limit; ++index)
    {
        ...
        stream << "<tr><td>" << value.test_case.id << "</td><td>"
               << escapeHtml(value.test_case.category) << "</td><td>" << value.test_case.width
               << QStringLiteral("×") << value.test_case.height << "</td><td>"
               << milliseconds(value.flood_fill.milliseconds) << "</td><td>"
               << milliseconds(value.run_parallel.milliseconds) << "</td><td>"
               << speedup(value.flood_fill.milliseconds, value.run_parallel.milliseconds)
               << "</td></tr>";
    }
    stream << "</tbody></table>";
}

void writeAllRows(QTextStream& stream, const std::vector<VpBitmapBenchmarkCaseResult>& results)
{
    stream << QStringLiteral("<h2>全部逐文件结果</h2>"
                             ...
                             "</tr></thead><tbody>");
    for (const VpBitmapBenchmarkCaseResult& value : results)
    {
        const VpBitmapVectorMetrics& metrics = value.run_parallel.metrics;
        stream << "<tr class=\"" << (value.passed ? "pass" : "fail") << "\"><td>"
               << value.test_case.id << "</td><td>" << escapeHtml(value.test_case.category)
               << "</td><td>" << value.test_case.width << QStringLiteral("×")
               << value.test_case.height << "</td><td>" << value.test_case.color_count
               << "</td><td>" << (value.passed ? "PASS" : "FAIL") << "</td><td>"
               << milliseconds(value.flood_fill.milliseconds) << "</td><td>"
               << milliseconds(value.run_serial.milliseconds) << "</td><td>"
               << milliseconds(value.run_parallel.milliseconds) << "</td><td>"
               << speedup(value.flood_fill.milliseconds, value.run_parallel.milliseconds)
               << "</td><td>" << metrics.run_count << "</td><td>" << metrics.segment_count
               << "</td><td>" << metrics.component_count << "</td><td>" << metrics.contour_count
               << "</td><td>" << metrics.svg_bytes << "</td><td>"
               << QString::number(metrics.estimated_working_bytes / 1024.0 / 1024.0, 'f', 2)
               << "</td><td>" << escapeHtml(value.validation_error) << "</td></tr>";
    }
    stream << "</tbody></table>";
}
                           ...
                           quint64 peak_working_set_bytes)
{
    ...
    {
        return VpResult<QString>::failure(toCoreText(QStringLiteral("无法写入 HTML 测试报告。")));
    }
    int passed = 0;
    double flood_ms = 0.0;
    double serial_ms = 0.0;
    double parallel_ms = 0.0;
    QMap<QString, VpAggregate> categories;
    std::vector<double> speedups;
    std::vector<const VpBitmapBenchmarkCaseResult*> valid_results;
    for (const VpBitmapBenchmarkCaseResult& value : results)
    {
        ...
        VpAggregate& aggregate = categories[value.test_case.category];
        ++aggregate.count;
        ...
        {
            speedups.push_back(value.flood_fill.milliseconds /
    ...
    const QString conclusion =
        all_passed && is_faster
            ...
            : QStringLiteral("尚不能声明性能提升：%1，累计多线程耗时%2四邻域基线。")
                  .arg(all_passed ? QStringLiteral("正确性通过") : QStringLiteral("存在正确性失败"),
                       is_faster ? QStringLiteral("低于") : QStringLiteral("不低于"));
    ...
    stream << QStringLiteral(
                  ...
                  "<title>位图矢量化 ")
           << results.size() << QStringLiteral(" 文件基准报告</title><style>")
           ...
           << QStringLiteral("</style></head><body><main><h1>位图矢量化批量正确性与性能报告</h1>"
                             "<p class=\"meta\">生成时间：")
           << QDateTime::currentDateTime().toString(QStringLiteral("yyyy-MM-dd HH:mm:ss"))
           << QStringLiteral("｜Qt ") << QT_VERSION_STR << QStringLiteral("｜线程：")
           << (options.thread_count > 0 ? options.thread_count : QThread::idealThreadCount())
           << QStringLiteral("｜系统：") << escapeHtml(QSysInfo::prettyProductName())
           << QStringLiteral("｜固定种子：") << options.seed
           << QStringLiteral("｜每方案中位数次数：") << options.repetitions
           << "</p><div class=\"verdict\"><b>" << escapeHtml(conclusion)
           << QStringLiteral("</b><br>逐文件加速比 P10 / P50 / P90：")
           << QString::number(percentile(speedups, 0.10), 'f', 2) << QStringLiteral("× / ")
           << QString::number(percentile(speedups, 0.50), 'f', 2) << QStringLiteral("× / ")
           << QString::number(percentile(speedups, 0.90), 'f', 2) << QStringLiteral("×</div>");
    writeSummaryCards(stream, passed, static_cast<int>(results.size()), flood_ms, serial_ms,
                      parallel_ms, peak_working_set_bytes);
    writeCategoryTable(stream, categories);
    writeWorstTable(stream, QStringLiteral("拆分多线程最慢的 20 个文件"), valid_results, false);
    writeWorstTable(stream, QStringLiteral("相对四邻域退化最明显的 20 个文件"), valid_results,
                    true);
    ...
    return VpResult<QString>::success(report_path);
}
