...
QByteArray contourHash(const std::vector<VpBitmapContour>& contours)
{
    ...
    for (const VpBitmapContour& contour : contours)
    {
...
QImage rasterize(int width, int height, const std::vector<VpBitmapContour>& contours)
{
    QMap<QRgb, std::vector<const VpBitmapContour*>> color_contours;
    for (const VpBitmapContour& contour : contours)
    {
    ...
    for (auto color_iterator = color_contours.cbegin(); color_iterator != color_contours.cend();
         ++color_iterator)
    {
        std::vector<std::vector<int>> intersections(static_cast<std::size_t>(height));
        for (const VpBitmapContour* contour : color_iterator.value())
        {
            for (std::size_t index = 0; index < contour->points.size(); ++index)
            {
                const QPoint& first = contour->points[index];
                const QPoint& second = contour->points[(index + 1) % contour->points.size()];
                if (first.x() != second.x())
                {
                    continue;
                }
                const int begin_y = std::max(0, std::min(first.y(), second.y()));
                const int end_y = std::min(height, std::max(first.y(), second.y()));
                for (int y = begin_y; y < end_y; ++y)
                {
                    intersections[static_cast<std::size_t>(y)].push_back(first.x());
                }
            }
        }
        for (int y = 0; y < height; ++y)
        {
            std::vector<int>& row = intersections[static_cast<std::size_t>(y)];
            ...
            for (std::size_t index = 0; index + 1 < row.size(); index += 2)
            {
                ...
                std::fill(pixels + begin_x, pixels + end_x, color_iterator.key());
            }
        }
    }
    return result;
...
bool compareImage(const QImage& source, const QImage& actual, QImage* difference)
{
    bool matches = true;
    if (difference)
    {
        *difference = QImage(source.size(), QImage::Format_ARGB32);
...
QString validateSvg(const VpBitmapVectorResult& result, const QString& algorithm)
{
    const auto parsed = parseSvgVectorData(result.svg_data);
    if (!parsed)
    {
        return QStringLiteral("%1 SVG 重新解析失败：%2").arg(algorithm, toQtError(parsed));
    }
    return {};
}
...
{
    VpBitmapValidationResult result;
    ...
    if (flood_fill.metrics.component_count != run_serial.metrics.component_count ||
        run_serial.metrics.component_count != run_parallel.metrics.component_count)
    {
        result.error = QStringLiteral("三种算法的四邻域色块数量不同。");
        return result;
    }
    if (result.flood_hash != result.serial_hash || result.serial_hash != result.parallel_hash)
    {
    ...
    const QString flood_svg_error = validateSvg(flood_fill, QStringLiteral("四邻域"));
    const QString serial_svg_error = validateSvg(run_serial, QStringLiteral("拆分单线程"));
    const QString parallel_svg_error = validateSvg(run_parallel, QStringLiteral("拆分多线程"));
    if (!flood_svg_error.isEmpty() || !serial_svg_error.isEmpty() || !parallel_svg_error.isEmpty())
    {
        result.error = flood_svg_error + serial_svg_error + parallel_svg_error;
        return result;
    }

    const QImage flood_image = rasterize(source.width(), source.height(), flood_fill.contours);
    if (!compareImage(source, flood_image, &result.difference))
    {
        result.error = QStringLiteral("四邻域轮廓回栅格结果与输入不同。");
        return result;
    }
    const QImage serial_image = rasterize(source.width(), source.height(), run_serial.contours);
    if (!compareImage(source, serial_image, &result.difference))
    {
        result.error = QStringLiteral("拆分法单线程回栅格结果与输入不同。");
        return result;
    }
    const QImage parallel_image = rasterize(source.width(), source.height(), run_parallel.contours);
