...
void appendBoundary(std::vector<bitmapVectorPrivate::VpBoundarySegment>& edges, int x, int y,
                    int direction, QRgb color, int component)
{
    if (direction == 0)
    {
        edges.push_back({{x, y}, {x + 1, y}, color, component, component});
    }
    else if (direction == 1)
    {
        edges.push_back({{x + 1, y}, {x + 1, y + 1}, color, component, component});
    }
    else if (direction == 2)
    {
        edges.push_back({{x + 1, y + 1}, {x, y + 1}, color, component, component});
    }
    else
    {
        edges.push_back({{x, y + 1}, {x, y}, color, component, component});
    }
}
                                                       ...
                                                       const VpBitmapVectorSettings& settings)
{
    ...
    std::vector<bitmapVectorPrivate::VpBoundarySegment> raw_segments;
    ...
    for (int start_y = 0; start_y < height; ++start_y)
    {
        for (int start_x = 0; start_x < width; ++start_x)
        {
            ...
            const QRgb color = effectiveColor(image.pixel(start_x, start_y), background, settings);
            if (qAlpha(color) == 0)
            {
            ...
            while (head < queue.size())
            {
                ...
                for (int direction = 0; direction < 4; ++direction)
                {
                    ...
                    const bool is_inside =
                        next_x >= 0 && next_y >= 0 && next_x < width && next_y < height;
                    const QRgb next_color = is_inside ? effectiveColor(image.pixel(next_x, next_y),
                                                                       background, settings)
                                                      : 0;
                    if (next_color != color)
                    {
                        appendBoundary(raw_segments, x, y, direction, color, component_count);
                        continue;
                    }
    ...
    if (!compressed)
    {
        return VpResult<VpBitmapVectorResult>::failure(toCoreText(toQtError(compressed)));
    }
    std::vector<bitmapVectorPrivate::VpBoundarySegment> segments = std::move(compressed.value());
    const qint64 connection_nanoseconds = connection_timer.nsecsElapsed();
    ...
    if (!contours)
    {
        return VpResult<VpBitmapVectorResult>::failure(toCoreText(toQtError(contours)));
    }
    VpBitmapVectorResult result;
    result.contours = std::move(contours.value());
    result.svg_data = bitmapContoursToSvgData(width, height, result.contours);
    ...
    result.metrics.estimated_working_bytes =
        ...
        static_cast<qint64>(raw_segments.size()) * sizeof(bitmapVectorPrivate::VpBoundarySegment) +
        static_cast<qint64>(segments.size()) * sizeof(bitmapVectorPrivate::VpBoundarySegment);
